...
# F6.4: PROVA do GUARD DE STALENESS NO UPGRADE do banco de leitura. O self-test
# pré-semeia um DB STALE (inválido) + sidecar de versão velho em documentDirectory
# (simula quem atualizou o app mas guardou a cópia ANTIGA) e prova que `ensureReadingDb`
# RE-COPIA o asset novo (mismatch de versão): Mateus (livro 40, o "Mateus 404 p/ quem
# atualiza") passa a ser consultável. adopted=true e matt1_verses>=1 vêm do RETORNO real
# de get_chapter (store local) — não hardcoded; aqui o script só confere os padrões.
MARK_DBUP="TLA_DBUP adopted=true"            # guard de upgrade: DB novo adotado após stale
...
# F5.7 (ADR-0039): PROVA DE PLANOS DE LEITURA no device. O app exercita a geração (F5.1) +
# o progresso (F5.4) via a fronteira `userdata::plans` (the_light_core via JSI) num dir de
# teste ISOLADO sob documentDirectory: list_reading_plans → reading_plan_day(0) →
# start_reading_plan("gospels", hoje) → reading_plan_day_index → set_reading_plan_completed →
# 2ª leitura INDEPENDENTE (persistência). O marcador é COMPOSTO do RETORNO real: plan_id/days
# do CATALOG (gospels=30), today_index/completed dos retornos, persisted=true (releitura). Aqui
# o script só confere padrões: plan_id="gospels", days=30, persisted=true.
MARK_PLANS='TLA_PLANS plan_id="gospels"'  # planos de leitura via a fronteira userdata::plans
...
echo "==> [5/5] Aguardando marcadores PT+EN + guard-upgrade (TLA_DBUP) + leitura + paralela + busca + xref + notas + ask + estudo + conversa + comparação + planos (até ${LAUNCH_WAIT}s)"
found=0
for _ in $(seq 1 "$LAUNCH_WAIT"); do
  if grep -qF "$MARK_PT" "$STREAM_LOG" \
    && grep -qF "$MARK_EN" "$STREAM_LOG" \
    && grep -qF "$MARK_DBUP" "$STREAM_LOG" \
    && grep -qE 'TLA_DBUP .*matt1_verses=[1-9][0-9]*' "$STREAM_LOG" \
    ...
    && grep -qE 'TLA_EXPORT .*has_attribution=true' "$STREAM_LOG" \
    && grep -qF "$MARK_PLANS" "$STREAM_LOG" \
    && grep -qE 'TLA_PLANS .*days=30' "$STREAM_LOG" \
    && grep -qE 'TLA_PLANS .*persisted=true' "$STREAM_LOG"; then
    found=1
    break
  fi
  sleep 1
...
echo "$MARK_PT"
echo "$MARK_EN"
grep -F "TLA_DBUP adopted=true" "$STREAM_LOG" | sed 's/.*\(TLA_DBUP\)/\1/' | head -1
