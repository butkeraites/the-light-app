...
- **Fatias RESTANTES — cada uma carrega UMA DECISÃO (não é refactor byte-a-byte trivial; por isso a fatia 1 parou na parte mecânica limpa).** Achado ao mapear:
  - **`headers`** (F-c) — ÚNICA restante byte-a-byte (o `Record` é ordem-insensível; o CORS via `anthropic_headers(key, browser=true)`). Baixo valor isolado (3 fns pequenas); candidata a ir junto de bodies.
  - **`extractors` (não-stream)** (F-b) — sucesso idêntico, MAS as mensagens de ERRO divergem: o core embrulha em `AiError::BadResponse` → `#[error("resposta inesperada do provedor: {0}")]`, enquanto o TS lança a string CRUA (`"o modelo recusou a solicitação (refusal)"`, `"resposta de texto vazia"`…). Migrar muda o texto de erro visível → precisa aceitar (é melhoria, mas é mudança).
  - **`bodies`** (F-c) — JSON semanticamente igual, mas a ORDEM das chaves difere (`serde_json::Value` = BTreeMap ordenado vs. `JSON.stringify` = ordem de inserção). Inócuo p/ a API; precisa confirmar que nenhuma guarda asserta o corpo string.
