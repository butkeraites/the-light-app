...
- **Fatias RESTANTES (mesma disciplina, cada uma revertível):** **F-b** extractors/deltas do transporte (nota: os deltas do core são ESTRITOS — erram em recusa/bloqueio mid-stream — vs. o TS LENIENTE atual; é mudança de comportamento a validar com golden-vector); **F-c-resto** bodies/headers (o header CORS via `browser=true`); **F-a** SQL — exige o NOVO módulo `pub query.rs` + enum `SqlParam` (nenhum existe hoje; toda query é `embedded`-gated sobre `&Connection`); **F-d** formatos (`notes::slug`/`parse_slug`, hoje privados/`embedded`). Status: **PROPOSTO → EM IMPLEMENTAÇÃO (fatia 1 de N entregue).**
