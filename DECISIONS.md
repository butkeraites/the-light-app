...
## ADR-0059 — Seam compartilhado de provedor/BYOK dos 4 painéis de IA (deep module) + recusa explícita de um mega-hook

- **Data:** 2026-07-06 · **Status:** aceito · **Tarefa:** arquitetura (deepening; `gate: false`) · **Depende:** **ADR-0023/0025** (BYOK/keystore, chave session-only no web), **F5.37** (`useConfiguredAiProviders`/`AiProviderNotice`), **F6.7** (Study/Chat des-mockados). **NÃO** toca o `the-light` nem `core/**` — 100% app-side (TS de UI + helpers puros).

**Contexto.** Os quatro painéis de IA (`ReaderAskPanel` 489, `ReaderChatPanel` 467, `ReaderStudyPanel` 792, `ReaderComparePanel` 452) eram quase-clones: cada um redefinia `MOCK_PROVIDER`/`PROVIDER_OPTIONS`, o bloco BYOK "lê a chave sob demanda e passa à fronteira", o render do chip de provedor + estilos, o efeito de checagem do cofre, o `onConfigureProvider` e o `err instanceof Error ? err.message : String(err)` (~24×). O `ReaderAskPanel` ainda re-inlinava a lógica do cofre por PREDATAR o hook `useConfiguredAiProviders`. Porém as FORMAS de chamada divergem de verdade: Ask = stream de resposta única (`askAnchoredStream`), Chat = multi-turno (`askSessionAnchored`), Study = dupla chamada `Promise.all([deepStudy, lexicalEntries])`, Compare = fan-out de N provedores lado a lado. Um "mega-hook" único brigaria com essas formas.

**Decisão.** Extrair o MENOR seam profundo que remove a maior duplicação com o menor risco, em vez de unificar as formas de chamada:
1. **`app/lib/errMessage.ts`** (puro) — `errMessage(err)` substitui as ~24 coerções (nesta entrega, os 4 painéis de IA; o restante como varredura mecânica posterior).
2. **`app/lib/aiProviders.ts`** (puro, DI) — `MOCK_PROVIDER`, `PROVIDER_OPTIONS_MOCK_FIRST`/`_MOCK_LAST`, `isMockProvider`, e `resolveProviderKey(provider, getKey) → KeyResolution` (`mock`|`key`|`no-key`) + `keyArg`. É **neutro de UX**: NUNCA lança por falta de chave e NUNCA loga/retorna o valor exceto em `kind:'key'`. Isso deixa a MESMA função servir o throw-shape (Ask/Chat/Study: `no-key` → erro i18n) E o cell-shape do Compare (`no-key` → coluna "sem chave"). Para `mock` faz curto-circuito SEM chamar `getKey` (offline, sem chave/rede).
3. **`app/components/ProviderPicker.tsx`** — `useProviderSelection(active)` (estado do provedor + checagem do cofre via `useConfiguredAiProviders` + derivações `isMock/providerHasKey/needsKey/showNoProviderNotice` + `loadKey()` que lança o erro i18n de needKey) e `<ProviderChips>` (o chip single-select idêntico + seus próprios estilos). Consumido só pelos 3 painéis single-select.
4. `useConfiguredAiProviders` ganha um `refresh()` (para o Ask reler o cofre após salvar chave inline). Compatível com os chamadores atuais.
5. **Recusa explícita** de um `useAnchoredAi` (Ask=stream vs Chat=multi-turno são estruturalmente diferentes) e de forçar o Compare (multi-select, badge/label distintos) no `<ProviderChips>`. O seam entrega SÓ seleção de provedor/chave — desconhece `AiAnswer`.

**Reconciliação com as leis.** *Offline-first/BYOK:* `resolveProviderKey` mantém `mock` sem chave/rede e a chave real lida sob demanda, nunca logada/persistida; a chave web segue session-only. *Anti-alucinação:* o seam NÃO toca `AiAnswer` — `citedText`/`passageText` (verbatim do store) seguem SEPARADOS da interpretação, renderizados em cada painel; `rewrite_anchors`/citação seguem no Rust `ai-pure`. *Zero drift:* nada de lógica de domínio Rust reimplementada em TS; nenhuma fronteira nova. *the-light:* intocado.

**Prova.** Suite headless nova `test:web:aiproviders` (node, sem browser/rede/chave) prova `resolveProviderKey`/`keyArg`/ordens/`errMessage` e o anti-vazamento (valor só em `kind:'key'`; `getKey` não chamado p/ mock). `tsc --noEmit` + as suites de fronteira (`ai`/`ai-stream`/`session`/`study`/`compare`/`lexicon`/`ai-multiprovider`) provam zero drift; `a11y-scan`/`a11y-modals`/`i18n`/`i18n-coverage` provam que mover o chip/`t()` para `ProviderChips` não regride a11y/rótulos; `contrast`/`theme`/`keystore` verdes. **Perf:** `test:web:perf-budget` re-baselineado DELIBERADAMENTE — ESTRUTURAL `moduleCount` 842 → 845 (+3 módulos eager: os 2 helpers puros + `ProviderPicker`); os BYTES do entry ficam DENTRO da banda existente (o LOC líquido CAIU apesar do +3 módulos), então só o `moduleCount` muda (espelhado em `scripts/measure-web-bundle.sh` + `loop/perf/web-bundle-budget.json`, nota ADR-0059).

**Consequências.** (+) *Locality:* o invariante offline/BYOK e a seleção de provedor vivem em módulos puros/testáveis, não em 4 cópias inline; um fix de key-load/chip/erro pousa em 1 lugar. (+) *Leverage:* uma interface, 3 painéis (chips/hook) + 4 painéis (helpers puros). (+) O `ReaderAskPanel` deixa de ser o caso-especial que predava o hook. (−) O Compare mantém deliberadamente seus próprios chips multi-select (badge `compare.byokBadge`, a11y `providerByok`) — documentado como "não forçar formas divergentes", não omissão. Relacionado: ADR-0023/0025 (BYOK), F5.37 (`useConfiguredAiProviders`), ADR-0047/0050 (política de re-baseline perf).

## ADR-0060 — Extrair `useChapterReader` + seam PURA `deriveVerseMarkers` da tela do capítulo

- **Data:** 2026-07-06 · **Status:** aceito · **Tarefa:** deepening de arquitetura (`gate: false`) · **Depende:** ADR-0049 (`useReaderModalA11y`, mesmo padrão de hook de leitura), ADR-0022 (formato notes/highlights). **Não** toca o `the-light` nem `core/**` — 100% app-side (TS). Sem mudança de comportamento, sem nova dependência, sem mudança de fronteira.

**Contexto.** `app/app/read/[book]/[chapter].tsx` (~580 linhas) concentrava 9 blocos `useState` e 7 efeitos de fetching de fronteira (translations, validade da 2ª tradução, passagem primária, passagem paralela, xrefs, dbPath, dataDir) MAIS `refreshUserData`, que buscava `listNotes`/`listHighlights` e os REDUZIA a um `Set<number>` de versículos com nota e um `Map<number,string>` de cores — lógica de shaping de dados presa na tela, não testável sem montar React. A tela ainda mantinha quatro estados paralelos (`askVerse`/`studyVerse`/`chatVerse`/`compareVerse`) que codificavam UM conceito (qual painel aponta para qual versículo).

**Decisão.** (1) Um hook `useChapterReader({book,chapter,translation,parallel,selectedVerse})` (`app/lib/useChapterReader.ts`) passa a possuir TODO o fetching e devolve o estado montado (`passage, secondaryPassage, translations, secondTranslation/setSecondTranslation, xrefs/xrefLoading/xrefError, dbPath, dataDir, notedVerses, highlightColors, refreshUserData, error`). (2) A redução de notas/highlights vira uma função PURA `deriveVerseMarkers(notes, highlights, book, chapter)` (`app/lib/verseMarkers.ts`), sem I/O/rede/wasm, provada por um teste headless node (`test:web:verse-markers`). (3) Os quatro `*Verse` colapsam em um único `activePanel: { kind, verse } | null` na tela. A tela retém apenas estado de CONTROLE (`translation`, `parallel`, `selectedVerse`, `activePanel`) e apresentação (título, `highlightedVerses`, `openXref`, JSX). O `openXref` PERMANECE na tela (faz `router.push`) e mantém a anotação `CrossRef['reference']` — por isso o `type CrossRef` segue importado na tela.

**Reconciliação com as leis.** *Offline-first/BYOK:* nada muda no transporte — o hook só orquestra as MESMAS chamadas de `app/web/reading` + `ensureReadingDb`/`ensureUserDataDir`; nenhuma rede/conta nova. *Anti-alucinação:* o texto do versículo continua vindo verbatim do store via fronteira; `deriveVerseMarkers` toca SÓ dado do usuário (presença de nota + nome de cor), nunca texto bíblico; `cited_text`/interpretação seguem separados nos painéis inalterados. *Zero drift:* nenhuma lógica de domínio Rust é reimplementada — a redução por-capítulo (filtrar book/chapter, `Single`-only) é a MESMA que já rodava inline em `refreshUserData`, agora isolada e testável. O `error` único (escrito pelos efeitos de passagem primária E paralela) foi movido VERBATIM, preservando a ordem de reset (`setPassage(null); setError(null)`).

**Prova.** `test:web:verse-markers` (novo, headless node) assere o recorte (só book/chapter corrente, só `Single`, última cor por versículo vence, refs estrangeiras/`Range`/`WholeChapter` fora, entradas vazias → conjuntos vazios). Paridade verificada por `tsc` verde e pelas suítes `notes`/`reading`/`search`/`xref` e guardas `a11y-scan`/`a11y-modals`/`i18n-coverage` (a estrutura dos painéis modais é inalterada — só mudam props `visible`/`verse`/`onClose`). O `activePanel` preserva a exclusividade que já existia de fato (cada painel abre pelo painel por-versículo, que fecha antes; só um modal por vez). **Perf:** re-baseline DELIBERADO — ESTRUTURAL `moduleCount` 845 → 847 (+2 módulos eager: o hook + a redução pura); BYTES do entry re-centrados nos valores medidos (2 exports idênticos/determinísticos: raw 1.359.705 / gzip 345.110 / brotli 271.513) — o código do hook (~2,4 KB gzip) estourou a banda antiga; espelhado em `scripts/measure-web-bundle.sh` + `loop/perf/web-bundle-budget.json`.

**Consequências.** (+) A tela some de ~9 estados + 8 efeitos + um `useCallback` para 4 estados de controle + apresentação; o fetching de leitura fica em um único ponto reusável e a redução vira uma seam pura testável sem montar React nem bootar wasm (*locality* + *leverage*). (−) Uma indireção a mais (hook) e um segundo arquivo `lib`; mitigado pela testabilidade e pela redução de superfície da tela. (−) O `activePanel` assume os 4 painéis mutuamente exclusivos (verdade hoje — cada um abre pelo painel por-versículo, que fecha antes); se um dia dois puderem abrir juntos, o modelo de slot único precisa revisão. Relacionado: ADR-0059 (mesma varredura de deepening), ADR-0047/0050 (política de re-baseline perf).

## ADR-0061 — Plumbing interno da fronteira UniFFI: helpers privados (`core_err`/`parse_lang`) + combinadores de store (`with_store`/`with_source`)

- **Data:** 2026-07-06 · **Status:** aceito · **Tarefa:** deepening de arquitetura (`gate: false`) · **NÃO** toca o `the-light` (rev `225b8c9`) — só consome o core pinado; mudança 100% interna a `core/src/lib.rs`.

**Contexto.** `core/src/lib.rs` é a fronteira UniFFI (delegadora fina do `the-light-core`). Os corpos nativos repetiam quatro formas mecânicas: mapeamento de erro `|e| CoreError::Generic { message: e.to_string() }` (58 fechamentos), `lang.parse::<Lang>().unwrap_or(Lang::Pt)` (10 sites), o prólogo `Store::open + EmbeddedSource::new + TranslationId::new` (nas fns de store) e a string de stub web (13 sites). Repetição = risco de drift e ruído que esconde a linha que de fato varia por função.

**Decisão (escopo desta entrega).** Introduzir uma camada de helpers **privados, NÃO exportados** (nenhum `#[uniffi::export]`/`#[derive(uniffi::…)]`), particionada por pureza de alvo:
- **cfg-livre:** `core_err(impl Display) -> CoreError` (aplicado aos 58 sites `.map_err(…)` → `.map_err(core_err)`) e `parse_lang(&str) -> Lang` (aplicado aos 10 sites). Ambos usados TAMBÉM pelas fns web-prepare (`ai-pure`) → compilados no wasm sem dead-code.
- **`cfg(not(target_arch = "wasm32"))`:** `with_store(db_path, |&Store| …)` e o açúcar `with_source(db_path, translation, |&EmbeddedSource, &TranslationId| …)`, aplicados às **7 fns puras de store** (`get_passage`, `list_translations`, `get_chapter`, `chapter_count`, `search`, `cross_refs`, `lexical_entries`) — concentram `Store::open` + mapeamento de erro + construção de source/tid num só lugar; o combinador PASSA a ser a própria fronteira cfg.

**Invariância dos bindings (crítico, PROVADO).** Nenhum helper é exportado → o `ubrn` extrai metadata só dos itens `#[uniffi::export]`, que ficaram inalterados. Verificado: `gen-bindings-web.sh` regerou os bindings e `the_light_app_core.ts`/`-ffi.ts`/`index.web.ts` ficaram **BYTE-IDÊNTICOS** ao snapshot anterior.

**Pureza do grafo wasm (ADR-0005/0010 preservados, PROVADO).** `with_store`/`with_source` nomeiam `Store`/`EmbeddedSource` (embedded-only) e por isso SÃO `#[cfg(not(target_arch = "wasm32"))]` — exatamente o gating dos corpos que substituem; `rusqlite`/store continuam fora do grafo wasm. Verificado: `gen-bindings-web.sh` (`cargo build --target wasm32-unknown-unknown`) exit 0 — o wasm compila puro. Efeito colateral: os helpers cfg-livres `core_err`/`parse_lang` entram no wasm e a de-duplicação encolheu o `frontierWasm` em 11 B (1.223.324 → 1.223.313; re-baseline determinístico em 2 exports, espelhado no orçamento perf); os bytes do ENTRY JS e o `moduleCount` (847) NÃO mudam (é 100% Rust).

**Fora de escopo (deferido, menor valor).** (1) De-duplicação da string de stub web (`web_unavailable`/`WEB_STORE_STUB`) — os corpos `cfg(wasm32)` NÃO são compilados/exercitados pelo `cargo test` nativo (só por `gen-bindings-web.sh`), então centralizar 13 strings ali rende pouco e some verificabilidade nativa; mantidas verbatim. (2) `with_store` nas fns de IA (`deep_study`/`ask_anchored`/`ask_anchored_stream`/`ask_session_anchored`) — já ganharam `core_err`/`parse_lang`; a conversão do prólogo de store é follow-up. (3) Os 26 `impl From` NÃO são macro-izados (legibilidade > concisão; risco de mapeamento silencioso errado). (4) `capture_prompts` (só 2 sites).

**Prova.** `cargo fmt --check` + `cargo clippy --all-targets -- -D warnings` + `cargo test` (80 passa, 0 falha — assertivas idênticas: nenhuma mudança de comportamento) + `gen-bindings-web.sh` exit 0 (wasm puro) + diff BYTE-IDÊNTICO dos bindings .ts + `test:web:perf-budget` verde (só `frontierWasm` re-baseado −11 B).

**Consequências.** (+) *Locality:* política de abertura de store, mapeamento de erro e default de idioma vivem em UM lugar; cada export DB-backed encolhe para a sua linha distintiva. (+) *Leverage:* os combinadores são a fronteira cfg — os chamadores param de repetir o prólogo `#[cfg(not(wasm32))]`. (−) Uma indireção de closure a mais nos corpos nativos (custo desprezível; retorno owned, sem escape de borrow). Relacionado: ADR-0005/0010 (matriz de features/gating por alvo), ADR-0059/0060 (mesma varredura de deepening).

## ADR-0062 — Colapsar o espelho nativo/web: relocar a costura para uma superfície DATA-ONLY no `the-light-core` (SQL/params + url/headers/body como DADO) — endpoint = **PR ao `the-light`** (HALT-gate); hoje só GUARDAS de drift

- **Data:** 2026-07-06 · **Status:** ✅ CONCLUÍDA (100% — 2026-07-10; sign-off dado, 11 PRs merged; ver ATUALIZAÇÃO/Conclusão abaixo) · **Tarefa:** deepening estratégico · **Depende:** ADR-0011 (espelho TS de query aceito como infra), ADR-0025 (transporte web `fetch` espelha `*_body`/`*_extract` PRIVADOS; **pré-sanciona** o PR: "aí sim um PR ao core torna `*_body`/`*_extract` `pub`"), ADR-0035 (precedente de PR ao `the-light`), ADR-0005 (matriz de features por alvo), ADR-0007 (resolver=v2 poda `embedded` no wasm32). Irmã das ADR-0059/0060/0061 (mesma varredura de deepening — estas app-side/ungated; esta, gated). **NÃO** implementa nada nesta sessão — para no gate.

**Contexto.** O alvo WEB **re-deriva em TS** lógica que o alvo NATIVO **delega** ao Rust. Achados (pin `225b8c929cf388e29dc148fec3975bf05a884b07`):
- **Transporte LLM (quarteto ×4):** `app/web/ai-anchored.web.ts` reimplementa `anthropicBody/openaiBody/geminiBody/ollamaBody` + `*Extract` + deltas de stream + headers + URLs, ESPELHANDO os `fn` PRIVADOS `*_body`/`*_chat_body`/`*_extract`/`*_stream_delta`/`messages_json`/`parse_api_response`/`api_error_msg` de `crates/the-light-core/src/ai/providers.rs`. Já puras/un-gated (compilam sob `ai-pure`), mas privadas; `DEFAULT_MAX_TOKENS = 8192` é `#[cfg(feature="embedded")]` + privado; URLs são literais dentro dos `impl` `embedded`-only.
- **9 SELECTs de SQL:** `sqlite-{reading,search,xref,lexicon}.web.ts` espelham `source/embedded.rs`, `search.rs`, `xref.rs`, `ai/lexicon.rs`. No core, essas queries vivem DENTRO de funções `embedded`-only que recebem `&Connection` e devolvem `rusqlite::Result` — a costura atual **não expõe o SQL como dado**.
- **Formatos de arquivo:** `userdata-fs.web.ts` (slug `notes/<ref>.md`, `highlights.json`) espelha `userdata/notes.rs::slug`/`parse_slug` (privados); `plans-fs.web.ts` espelha o serde de `userdata/plans.rs`.
- **Constantes hard-coded:** `8192` (the-light, cfg embedded/privado), `1`/`20` (the-light, **já pub**), `32`/`4` (`DEFAULT_LEXICON_LIMIT`/`DEFAULT_RESEARCH_LIMIT`) — estas DUAS vivem no **boundary do app** (`core/src/lib.rs`), **não** no the-light.

**Riscos de drift observados (hoje).** (a) Comentários dos espelhos SQL citam `rev 8f66004`, mas o pin real do `core/Cargo.toml` é `225b8c9…` — âncora de rev DESATUALIZADA. (b) `32`/`4` são espelhados como "do core" nos comentários, mas a fonte única é o boundary do app — paridade correta hoje, sem guarda automática.

**Decisão (endpoint zero-drift).** Relocar a costura para uma superfície **DATA-ONLY** no `the-light-core`, transformando o espelho TS de RE-IMPLEMENTAÇÃO em EXECUTOR BURRO:
1. **Transporte:** tornar `pub` as helpers puras de `providers.rs` (`*_body`, `*_chat_body`, `*_extract`, `*_stream_delta`, `messages_json`, `parse_api_response`, `api_error_msg`, `sse_data`, `with_stream`); **des-gate** e tornar `pub` `DEFAULT_MAX_TOKENS`; hoistar URLs para `pub const` un-gated; adicionar `pub fn {provider}_headers(key)->Vec<(String,String)>`. O wasm passa a IMPORTAR `{url, headers, body}` e a chamar `{provider}_extract`/`_stream_delta` — o `fetch` no TS vira transporte puro.
2. **Store:** novo módulo `pub` `query.rs` com planners puros `search_plan/xref_plan/chapter_plan/lexicon_plan(args) -> (sql, Vec<SqlParam>)` e `pub const *_SELECT`, onde **`SqlParam` é um enum PURO do core** (`Text(String)`/`Int(i64)`), NUNCA `rusqlite::types::Value`. O caminho `embedded` ganha um `impl From<SqlParam> for rusqlite::types::Value` (único ponto que toca rusqlite, `cfg(embedded)`). O TS executa `{sql,params}` no `wa-sqlite`+OPFS e aplica row-shapers espelhados de `pub fn shape_*_row`.
3. **Formatos:** tornar `pub` `notes::slug`/`parse_slug`; expor constantes de nome de arquivo; reusar DTOs serde já `pub` dos planos.
Fim-de-jogo: UMA fonte da verdade (SQL, corpo de request, constantes, ranking/clamp) em Rust; TS com ZERO lógica de domínio; nativo e web só podem CONCORDAR.

**Por que é um HALT-gate.** Nada acima cabe no app: exige editar o repositório PINADO `the-light` (novo `pub`, des-gate de `DEFAULT_MAX_TOKENS`, módulo `query.rs`, enum `SqlParam`). Regra não-negociável: **o `the-light` só muda via PR + ADR**. Esta ADR PROPÕE; a implementação abre o PR e re-pina o rev — sign-off humano obrigatório.

**Argumento de segurança-wasm.** Tornar esses itens `pub` NÃO arrasta deps não-wasm-safe para o grafo wasm, DESDE QUE a superfície seja DATA-ONLY. As helpers de transporte são puras sobre `serde_json::Value`/`&str` (não referenciam `reqwest`); `DEFAULT_MAX_TOKENS` é `u32`; URLs são `&'static str`. O ÚNICO vetor de risco é assinatura que exponha `rusqlite::Connection`/`reqwest::RequestBuilder`/`rusqlite::types::Value` (o `search.rs` atual monta `params: Vec<rusqlite::types::Value>`) — por isso a costura INTRODUZ o enum próprio `SqlParam` e planners que devolvem `(String, Vec<SqlParam>)`, isolando o mapeamento para `rusqlite` sob `cfg(embedded)`. Tornar itens PUROS `pub` não altera a seleção de features (só a visibilidade): a matriz de alvo (ADR-0005) e o resolver=v2 (ADR-0007) seguem podando `embedded`/rusqlite/reqwest no wasm32. **Portão de prova obrigatório no CI do PR:** `cargo build -p the-light-core --no-default-features --features ai-pure --target wasm32-unknown-unknown` deve compilar SEM `rusqlite`/`reqwest`.

**Anti-alucinação & BYOK preservados.** Texto do versículo continua VERBATIM do store; `cited_text`/`passage_text` seguem SEPARADOS da interpretação; a reescrita de âncoras de citação continua em Rust (`ai/citation.rs`, `ai-pure`), NUNCA reimplementada em TS. A chave BYOK web segue session-only/in-memory, só no header (ADR-0025/0058); a costura move a MONTAGEM do request para o Rust, mas `fetch` e chave permanecem client-side.

**Migração faseada (SÓ após o the-light expor a costura).** (F-a) SQL: trocar cada `*_SELECT` literal do TS por import do `pub const` + planner; a guarda de paridade vira invariante. (F-b) extractors/deltas: TS chama `{provider}_extract`/`_stream_delta` via wasm; remove `*Extract`/`*Delta` do TS. (F-c) request bodies/headers/urls do wasm. (F-d) formatos: slug/parse via wasm. Cada fatia: golden-vector cross-repo + budget verde + flag de reversão por uma fatia.

**O que dá para melhorar HOJE (sem tocar o the-light) — GUARDAS, não o fix (task separada NÃO-gated).** (1) Corrigir os comentários de rev `8f66004`→`225b8c9` APÓS provar (via guarda) que o SQL não divergiu. (2) `app/web/__tests__/mirror-drift.web.test.mjs`: lê o pin de `core/Cargo.toml`, faz `git -C ../the-light show <rev>:…`, extrai cada SELECT/const e ASSERTA contra o export TS. (3) `app-const-parity.web.test.mjs`: assere `DEFAULT_LEXICON_LIMIT`(32)/`DEFAULT_RESEARCH_LIMIT`(4) de `core/src/lib.rs` == os valores TS (drift APP-owned, fechável hoje). (4) Tripwire de pin: `MIRROR_PIN` versionado; falha quando `core/Cargo.toml` rev != `MIRROR_PIN`. **Estas guardas DETECTAM drift; não ELIMINAM o espelho — o fix é o PR + migração (gated).**

**Consequências.** (+) Fonte única da verdade para SQL/transporte/constantes; TS sem lógica de domínio; nativo/web só concordam; bundle web tende a DIMINUIR. (−) Acopla o app a um novo rev do the-light (re-pin + rebuild wasm/bindings); amplia a superfície `pub` do core (contrato a manter); exige o enum `SqlParam` no core. (Rejeitado) relocar o drift para um "core do app" que copie o the-light — violaria "não reimplementar lógica de domínio" e a regra de fonte única.

**Ação requerida (gate).** Sign-off humano nesta ADR → abrir o PR `feat/data-seam-for-web-parity` no `the-light` → re-pinar AMBOS os blocos `the-light-core` em `core/Cargo.toml` (ai-pure + `[target cfg(not wasm32)]` embedded) → regen bindings → migração faseada F-a→F-d. As 4 guardas ungated podem virar uma task própria ANTES do PR (endurecem o espelho enquanto ele existe).

**ATUALIZAÇÃO (2026-07-10) — sign-off dado; EM IMPLEMENTAÇÃO por fatias.** O usuário autorizou o PR ao `the-light` e o merge. Progresso:
- **the-light — PR `feat/data-seam-for-web-parity` (`#6`) MERGED** (rev `76636af`→`4c76d6f`). Expõe a **superfície DATA-ONLY de transporte** (`providers.rs`): `pub` nas helpers puras (`*_body`/`*_chat_body`/`*_extract`/`*_stream_delta`/`messages_json`/`sse_data`/`with_stream`/`parse_api_response`/`api_error_msg`); des-gate + `pub` `DEFAULT_MAX_TOKENS`; NOVOS `pub const`/`fn` un-gated `ANTHROPIC_URL`/`OPENAI_URL`/`OLLAMA_DEFAULT_HOST`/`ollama_url`/`gemini_url`/`*_headers` (o header de CORS do browser da ADR-0058 é parametrizado). O caminho nativo consome as MESMAS helpers (fonte única; 198 testes verdes, byte-a-byte). **Novo job CI `wasm-pure`** (`cargo build --features ai-pure --target wasm32`) — o portão wasm-safe mandado por esta ADR, agora guardando o core na fonte.
- **App — fatia 1 (transporte: URLs + `max_tokens`) MIGRADA.** Re-pin dos 2 blocos → `4c76d6f`; frontier `#[uniffi::export]` cfg-free `llm_default_max_tokens()` + `llm_endpoint_url(provider, model, stream, host?)` (delegam à superfície `pub` do core); regen de bindings local (wasm-safe provado pelo build do wrapper). `ai-anchored.web.ts` deixou de hard-codar `8192` e as 4 URLs — passa a IMPORTAR da fronteira (SÍNCRONO; só no transporte, após wasm pronto). Byte-a-byte confirmado pelas guardas `ai`/`ai-stream`/`ai-multi`/`ai-multiprovider`/`study`/`compare` (mesmos endpoints). Guarda `mirror-drift`: entrada `DEFAULT_MAX_TOKENS` RETIRADA (espelho colapsado → sem drift a guardar).
- **Fatias RESTANTES — cada uma carrega UMA DECISÃO (não é refactor byte-a-byte trivial; por isso a fatia 1 parou na parte mecânica limpa).** Achado ao mapear:
  - **`headers` + `bodies` (F-c) — FEITO na fatia 2 (app PR #44).** Frontier `llm_request_headers(provider, key, browser)` (+Record `LlmHeader`) e `llm_request_body(provider, model, system, user, stream)` (delegam a `*_headers`/`*_body`+`with_stream` do core; `body.to_string()` compacto). `ai-anchored.web.ts`: os `*Body`/`*Headers` viraram ADAPTADORES FINOS (zero lógica). Confirmado seguro: as guardas fazem `JSON.parse(body)` e asseram CAMPOS (`.model`/`.max_tokens`/`.stream`/`.system_instruction`), então a ordem de chaves (serde vs JSON.stringify) é invisível; o CORS do Anthropic vai via `browser=true`. `ai`/`ai-stream`/`ai-multi`/`ai-multiprovider`/`study`/`compare` verdes.
  - **`extractors` + `stream deltas` (F-b) — FEITO na fatia 3 (app PR #45).** Frontier `llm_extract(provider, response_json)` (parseia via `parse_api_response` + `*_extract`) e `llm_stream_delta(provider, line)` (`sse_data`+`*_stream_delta` p/ SSE; linha inteira p/ ollama; `Option<String>` = emitir/pular, `Err` = abortar). `ai-anchored.web.ts`: os 4 `*Extract`, 4 `*Delta` e o `parseSseData` (≈189 linhas de lógica de domínio) REMOVIDOS; o laço de streaming ficou uniforme (`llmStreamDelta`). Mudanças ACEITAS pelo usuário: (a) texto de erro do extractor agora com prefixo do core; (b) streaming ESTRITO — recusa/bloqueio/erro in-band mid-stream agora ABORTA (antes truncava em silêncio). As guardas `ai`/`ai-stream`/`ai-multi`/`ai-multiprovider`/`study`/`compare` verdes (streams limpos ⇒ mesmos tokens); o comportamento estrito é testado nos `stream_tests` do próprio the-light.
  - **`stream deltas`** (F-b) — o core é ESTRITO (erra em recusa/bloqueio/`error` mid-stream) vs. o TS LENIENTE (engole e trunca). Mudança de comportamento a validar com golden-vector.
  - **`SQL` (F-a) — FEITO (leitura/busca/xref/passage) via the-light PR #8 + app PR #47.** the-light: novo módulo un-gated `query` (`ai-pure`) — `SqlParam` (enum próprio Text/Int, NUNCA `rusqlite::Value`; `From<SqlParam> for Value` isolado sob `#[cfg(embedded)]`) + consts `*_SELECT` + planners `chapter_count_plan`/`has_translation_plan`/`translations_plan`/`passage_plan`/`search_plan`/`xref_plan`; os módulos nativos (`embedded`/`search`/`xref`) passaram a USAR os planos (fonte única, 202 testes byte-a-byte). app: re-pin → `ff6c988`; frontier `#[uniffi::export]` `chapter_query`/`chapter_count_query`/`translations_query`/`has_translation_query`/`passage_query`/`xref_query`/`search_query`/`build_match_query` (Record `SqlPlan{sql, params: Vec<SqlParam>}`); `sqlite-{reading,search,xref}.web.ts` + `sqlite.web.ts` deixaram de espelhar SELECT/params — executam `{sql, params}` da fronteira via `bindPlanParams` (posicional). mirror-drift: 6 SELECTs de leitura/busca/xref/passage RETIRADOS (colapsados). Verde: tsc + run-guards all (51/51) + reading/search/xref/passage guards byte-a-byte.
  - **`SQL` — LÉXICO (`collect`/`interlinear`/`attributions`) — FEITO na fatia lexicon (the-light PR #9 + app PR #49).** the-light: `query` ganhou `lexicon_collect_plan`/`interlinear_plan`/`attributions_plan` (+ os `*_SELECT`) e recebeu `base_strong`/`resolve_verses` (puros, MOVIDOS de `ai::lexicon`); `ai/lexicon.rs` passou a CONSUMIR os planos via `params_from_iter` (`params!`/`VerseRange` viraram test-only), 206 testes byte-a-byte. app: re-pin → `80aa1a7`; frontier cfg-free `lexicon_collect_query`/`interlinear_query`/`attributions_query`/`base_strong` (Record `SqlPlan`); `sqlite-lexicon.web.ts` deixou de espelhar os 3 SELECTs + o `baseStrong` local — EXECUTA `{sql, params}` da fronteira via `bindPlanParams`, mantendo SÓ o SHAPER (não-SQL) de agregação por Strong base ("primeiro não-nulo vence"); mirror-drift `interlinear tokens` APOSENTADO (`SELECTS = 0`, todos os espelhos de SELECT colapsados). `DEFAULT_LEXICON_LIMIT` segue APP-owned. Verde: tsc + run-guards all (51/51) + `lexicon`/`interlinear` byte-a-byte (João 3:16 → 21 Strong / 26 tokens).
  - **`formatos` (F-d) — FEITO (the-light PR #7 + app PR #46).** the-light: novo módulo un-gated `userdata::note_slug` (`pub slug`/`parse_slug`, `ai-pure`) que o `notes.rs` (fs) passou a importar — fonte única. app: re-pin → `8c80bda`; frontier `note_slug(reference)`/`parse_note_slug(stem)` (+ helper `to_core_reference`); `userdata-fs.web.ts`: `slugForNote` delega a `noteSlug`, o parse do `listNotesFs` usa `parseNoteSlug`. Byte-a-byte (a guarda `notes` confirma o slug `John_3.16.md` idêntico ao nativo). `formatReferenceEn` fica só para o `ref` dos highlights (fatia à parte, não-slug).
- **Conclusão (2026-07-10, após 11 PRs — 4 ao the-light, 7 ao app, todos verdes):** os TRÊS seams da ADR foram **100% colapsados** ao endpoint DATA-ONLY:
  - **Transporte — 100%.** URLs, `max_tokens`, headers, corpo do request, extractor da resposta e parser de delta do stream vêm todos do core; `ai-anchored.web.ts` perdeu ~370 linhas de lógica de domínio. (Mudanças de comportamento aceitas: texto de erro do extractor com prefixo do core; streaming ESTRITO.)
  - **Formatos — 100%.** `note_slug`/`parse_slug` único no core (`userdata::note_slug`).
  - **SQL — 100% (leitura/busca/xref/passage + LÉXICO).** Todas as consultas do store vêm do módulo `query` (`SqlParam` + planners) no core; o web executa `{sql, params}` via `bindPlanParams`. A fatia final (LÉXICO: `collect`/`interlinear`/`attributions` + `base_strong`, the-light #9 / app #49) retirou o ÚLTIMO espelho de SELECT do TS — `mirror-drift` agora guarda `SELECTS = 0` (só restam constantes numéricas app-owned, ex.: xref votos/limite). O único código de léxico que segue em TS é o SHAPER de agregação (não-SQL), sancionado pela ADR-0011.
  - **Portão wasm-safe** (`wasm-pure`) no CI do the-light; anti-alucinação/BYOK preservados em todas as fatias; `DEFAULT_LEXICON_LIMIT`/`DEFAULT_RESEARCH_LIMIT` seguem app-owned. Status: **✅ CONCLUÍDA — ADR-0062 = 100% (transporte + formatos + SQL, inclusive léxico, entregues).** Fim-de-jogo atingido: UMA fonte da verdade (SQL, request, constantes, ranking/clamp, `base_strong`) em Rust; o TS web não carrega mais lógica de domínio nem espelho de SELECT — nativo e web só podem CONCORDAR.

## ADR-0063 — Design language "Vigil" (redesign, Fase 1): camada de tokens (paleta ESCURA-primeiro + escalas tipografia/espaço/raio/elevação/movimento) e paleta de leitura SÉPIA, guardadas pelo contraste AA

- **Data:** 2026-07-06 · **Status:** ACEITO · **Tarefa:** redesign de UI (Fase 1 do plano "Vigil") · **Depende:** ADR-0015 (tema + tokens de cor), ADR-0043 (persistência do modo; `THEME_MODES = [light, dark]`), ADR-0046 (guarda de contraste WCAG AA headless). **Aprovado pelo usuário:** direção visual **Vigil** (escura-primeiro), navegação **single-stack** (sem tabs) e **serifa de leitura bundlada** — as duas últimas materializam em fases seguintes.

**Contexto.** A engenharia do app é forte, mas a camada VISUAL era um protótipo funcional: existiam só **tokens de COR** (14, com guarda AA), **sem** escala de tipografia/espaço/raio/elevação/movimento, **sem** fonte de leitura e **sem** customização de leitura — mesa-posta para um leitor de Bíblia. Para competir por adoção via UI, o usuário aprovou a direção **Vigil**: reverente, escura-primeiro (ouro de vela + pergaminho sobre tinta profunda; "a Luz nas trevas"), com modo claro refinado ("papel quente") e um tema de leitura sépia.

**Decisão (100% app-side, aditiva).**
1. **Re-tematizar** `LIGHT`/`DARK` para Vigil e **adicionar `SEPIA`** como paleta de **LEITURA** (NÃO um `ThemeMode` — a guarda de persistência exige `THEME_MODES=[light,dark]`; sépia é selecionável no leitor numa fase seguinte). Novos tokens SÓLIDOS (`#rrggbb`, exigência da guarda): `surface`, `surfaceElevated`, `selectionBg` (banho de ouro PRÉ-MESCLADO), `onAccent`, `success`. Como todo componente lê `colors` por TOKEN (zero hex literal), trocar os valores **re-tematiza o app inteiro** sem tocar componentes.
2. **Nova camada de tokens NÃO-cromáticos**, PURA (sem `react-native`), em `app/lib/tokens/{space,radius,type,elevation,motion}.ts` (+ barril). `type.ts` define o RAMO de papéis (display/title/heading/verse/verseNumber/body/label/caption/button) e resolve a **família serifa por plataforma** (Palatino no iOS · `'serif'`=Noto Serif no Android · stack CSS no web) — a serifa do **sistema** já eleva a leitura nos 3 alvos **sem nova dependência**; a Literata bundlada (expo-font) entra depois.
3. **`theme.ts`** expõe `space`/`radius`/`type`(resolvido)/`elevation`/`motion` via `useTheme()`, **aditivamente** — os consumidores de `const { colors } = useTheme()` seguem intactos. Re-exporta `SEPIA`/`READING_PALETTES`.
4. **Guarda de contraste estendida** (ADR-0046): novos pares BLOQUEANTES (`text/surface`, `verseText/surface`, `verseText/selectionBg`, `onAccent/accent`, `success/background`) + auditoria da paleta **SÉPIA**. Todos passam AA nas 3 paletas (claro/escuro/sépia) com folga; sépia `accent` fixado em `#6e520c` p/ atingir 4.5:1 sobre o fundo sépia.

**Restrições preservadas.** Offline-first (nenhuma rede/telemetria). Anti-alucinação: isto é CROMO de UI puro — o versículo segue VERBATIM do store via `verseText`, jamais tocado. Fonte única: nenhuma lógica de domínio em TS. `the-light` intocado (mudança 100% app-side, sem re-pin). **Guardas verdes:** `tsc --noEmit`; 28 testes web headless; contraste (14 pares × 2 modos + 14 sépia = todos AA); persistência de tema.
...
**Consequências.** (+) Busca deixa de falhar em silêncio (rescata via "did you mean?"), ganha autocomplete e equivalências, e casa o idioma do leitor. (−) O mapa de sinônimos/conceitos exige **curadoria manual** (a semente cobre pouco; cresce sob demanda).

**Fase B (implementada — autocomplete de TERMO do corpus).** `scripts/gen-search-wordlist.mjs` deriva `app/assets/data/wordlist.{pt,en}.json` (palavras de conteúdo por idioma, `[display, freq]` ORDENADO pela forma dobrada; determinístico; entra em `gen-reading-sample-db.sh`). Runtime: `searchWordlistIndex` (busca binária de prefixo, PURA) + `searchWordlist` (import LAZY do asset — chunk async no web, fora do 1º paint; degrada a nada se ausente). A tela mostra completações enquanto se digita UM token ("eter" → eternamente/eternidade…); as palavras vêm do texto da tradução buscada → tocar numa SEMPRE retorna resultados (sem sondar). Assets **commitados** (pequenos ~556 KB, estáveis) e regeneráveis. Verificado no browser: EN "eter"→eternal/eternity; PT "eter"→eterna/eternamente/eterno/eternidade/eternas/eternos; "eterni"→eternidade/eternidades. Prova headless `search-smart` cobre o índice puro + integração com o asset PT real (paridade fold gerador↔runtime).

## ADR-0065 — Lookup de passagem na home: seletor de VERSÃO + RANGES/LISTAS, app-side (sem tocar o core)

- **Data:** 2026-07-06 · **Status:** ACEITO · **Tarefa:** melhoria do lookup da home (aprovada) · **Depende:** ADR-0014 (`parse_reference`/`get_chapter`), ADR-0063 (Vigil), ADR-0064 (default de tradução por idioma). Display escolhido: **inline na home**.

**Contexto.** O campo "Digite uma passagem" (a) vinha **só em inglês** (web `getPassage` default KJV; **nativo nem lia texto** — só `parseReference`) e **sem seletor de versão**; (b) o core PINADO só parseia UMA referência (verso / range-no-capítulo / capítulo inteiro; `:`/`.` e `-`), **rejeitando** ranges de capítulo ("João 3-4"), cross-chapter, ranges de livro e **LISTAS**. O usuário quer ranges (versos→capítulos→livros) e listas.

**Decisão (100% app-side, POR CIMA de `parseReference`/`getChapter` INALTERADOS).**
1. **Seletor de versão** reusa o padrão da busca (pt→Almeida / en→KJV, reativo/validado). O TEXTO passa a vir de `getChapter(translation, book, chapter)` + **filtro de versos app-side** — funciona nas DUAS plataformas (corrige o web-só-KJV e o nativo-sem-texto) e contorna o limite de "verso único" do store web (`readPassage`).
2. **`passageQuery.ts` (PURO):** quebra listas (`;`/`,`/nova linha) e classifica cada item (ref / chapterRange / crossChapter / bookRange / wholeBook / invalid; hífen `-` e travessão `–`). **`passageResolve.ts` (async, deps injetadas):** expande em leituras ATÔMICAS de capítulo (nome de livro via `parseReference("<livro> 1")` — reusa os aliases do core; contagem via cânon `listBooks().chapterCount`), MEMOIZA `getChapter` por (livro,capítulo), recorta bordas de cross-chapter, e marca `truncated` além de um TETO (~15 cap / ~200 versos). **`PassageResultView`** renderiza os trechos num cartão ROLÁVEL (serifa Vigil, cabeçalho por referência).

**Restrições preservadas.** Core/mirror **intocados**. Anti-alucinação: texto VERBATIM de `getChapter`; o app só SELECIONA versos e monta rótulos (chrome). Offline. a11y (Pressable único/role/label/≥44) + i18n pt/en. **PERF:** a leitura pesada (`reading`/`db`) é importada LAZY (chunk async, fora do 1º paint) — `parseReference` (leve) e a lógica pura seguem eager; **orçamento web verde**.
...
## ADR-0069 — Síntese temática sobre MÚLTIPLOS trechos disjuntos do Escopo de Estudo — caminho de FIDELIDADE = **PR ao `the-light`** (HALT-gate); interino app-side possível (reduce sobre o fan-out)

- **Data:** 2026-07-07 · **Status:** PROPOSTO (aguarda sign-off humano — `gate: true` para o Caminho A) · **Tarefa:** Escopo de Estudo, Fase 5 · **Depende:** do Escopo de Estudo (multi-seleção no leitor/busca/painel-de-verso + persistência de sessão — `studyScope.ts`/`useStudyScope.ts`), da fronteira de IA ancorada (`ask_anchored`/`ask_anchored_stream` recebem uma STRING de referência que o core parseia como Single/Range-intra-capítulo/WholeChapter; `deep_study`/`ask_session_anchored`/`lexical_entries` recebem book/chapter/verse NUMÉRICOS → só Single/WholeChapter), ADR-0035 (precedente de PR ao `the-light`), ADR-0008 (anti-alucinação: texto do versículo VERBATIM do store, o LLM só interpreta), ADR-0059 (seam BYOK de provedor). Irmã filosófica da ADR-0062 (gated, PR ao core). **NÃO** implementa o Caminho A nesta sessão — para no gate.

**Contexto.** O usuário pediu um estudo "não tão subscrito a um texto específico, mas ao TEMA" — reunir múltiplos parágrafos/capítulos/faixas e estudar o conjunto. A Fase 4 entregou o lado da SELEÇÃO (montar o escopo multi-trecho a partir do leitor, da busca e do painel-de-verso, com persistência). Falta o lado da RESPOSTA: hoje, quando o escopo tem vários trechos DISJUNTOS, o `ScopeStudySheet` faz **fan-out** — N chamadas `ask_anchored` independentes → N respostas separadas, cada uma com seu texto citado VERBATIM do store (anti-alucinação preservada, mas SEM tecer os trechos). Um só trecho contíguo já vira UMA resposta conjunta (via `ask_anchored_stream`, que aceita Range). O que falta é UMA reflexão TECIDA sobre trechos disjuntos.

**Restrição de fronteira (a raiz do gate).** TODA chamada de IA do core ancora em UMA única passagem: `ask_anchored`/`_stream` recebem UMA string de referência (Single | Range intra-capítulo | WholeChapter — o core resolve o verbatim daquele trecho e monta o prompt); `deep_study`/`ask_session_anchored` recebem book/chapter/verse numéricos (Single/WholeChapter). **Não existe** chamada "livre" nem multi-âncora. Logo, um prompt CONJUNTO que resolva o verbatim de N trechos disjuntos e os sintetize numa só resposta **não cabe no app** — exige nova função na fronteira UniFFI do `the-light`.

**Decisão proposta — dois caminhos (o usuário escolhe; não são mutuamente exclusivos).**

- **Caminho A — síntese conjunta REAL via core (GATE, maior fidelidade).** Nova `pub fn ask_multi_anchored(db, translation, refs: Vec<String>, question, provider, key, model, lang) -> AiAnswerMulti` (e a variante `_stream`) em `the-light-core::ai`. O CORE resolve o verbatim de CADA `ref` do store (reusando `parse_reference` + `EmbeddedSource::passage`), monta UM prompt com todos os trechos citados e devolve `{ citedPassages: Vec<CitedPassage>, interpretation }` — a interpretação TECE os trechos; cada `CitedPassage` mantém seu texto VERBATIM separado. A UI mostra os N blocos citados (verbatim, régua dourada) acima de UMA `InterpretationBlock` unificada. **Anti-alucinação intacta por construção** (o verbatim vem do store, no core; o modelo só interpreta). Exige: PR ao `the-light` (novo `pub fn`, wasm-safe — só resolve texto do store + monta prompt, sem novas deps não-wasm), re-pin do rev em `core/Cargo.toml`, regen de bindings, espelho web (`reading.web.ts`). Prova wasm obrigatória no CI do PR (compilar `--features ai-pure --target wasm32` sem rusqlite/reqwest).

- **Caminho B — reduce temático app-side sobre o fan-out (SEM tocar o core, interino).** Após o fan-out já existente (N respostas, N interpretações do LLM), UMA chamada extra `ask_anchored` — ancorada no PRIMEIRO trecho do escopo (que É parte legítima da seleção) — cuja `question` carrega as N interpretações anteriores + a instrução "teça uma síntese temática conectando estas reflexões". A síntese opera sobre INTERPRETAÇÕES (saída do modelo), NÃO sobre texto bíblico cru — os versículos VERBATIM continuam exibidos nos blocos do fan-out acima; a síntese é rotulada como `InterpretationBlock` ("Síntese temática"). **Anti-alucinação NÃO é violada** (nenhum verbatim entra pela `question`; nada de Escritura vem do modelo). **Porém** é arquiteturalmente IMPURO: a âncora é arbitrária (um trecho representa o todo) e a montagem do prompt de síntese migra para o TS — exatamente o tipo de re-derivação que a ADR-0062 quer ELIMINAR. Serve como ponte se quisermos entregar valor antes do PR.

**Anti-alucinação & BYOK.** Nos dois caminhos: texto de versículo VERBATIM do store (Caminho A: resolvido no core; Caminho B: já exibido pelos blocos do fan-out); interpretação SEMPRE rotulada e separada (`CitedText` vs. `InterpretationBlock`); chave BYOK lida sob demanda, nunca logada; atribuições CC-BY intactas. A diferença é de PUREZA arquitetural (fonte única no Rust vs. montagem de prompt no TS), não de segurança do contrato anti-alucinação.

**Por que o Caminho A é um HALT-gate.** Cria `pub fn` novo no repositório PINADO `the-light` + re-pin do rev. Regra não-negociável: **o `the-light` só muda via PR + ADR + sign-off humano**. Esta ADR PROPÕE; a implementação abre o PR `feat/ask-multi-anchored` e re-pina — sign-off obrigatório.

**O que dá para fazer HOJE sem o gate.** (1) O Caminho B (interino), claramente rotulado como "síntese sobre as reflexões", com um toggle que preserva o fan-out puro. (2) Polir a apresentação do fan-out (agrupar por tema, ordenar, rótulos). Ambos app-side, sem tocar o core.

**Recomendação.** Se a fidelidade do estudo temático importa (e a fala do usuário sugere que sim), **Caminho A** é o certo — é a fonte-única correta e não tem o cheiro de re-derivação da ADR-0062. O Caminho B só se quisermos uma ponte imediata antes de agendar o PR ao core; nesse caso, deixá-lo atrás de rótulo honesto ("síntese das reflexões", não "leitura conjunta") e trocá-lo pelo A quando o PR aterrissar.

**Consequências.** (+A) Estudo temático de alta fidelidade, fonte única no Rust, anti-alucinação por construção. (−A) Re-pin do the-light + regen de bindings + espelho web; amplia a superfície `pub` do core. (+B) Entrega hoje, sem gate, anti-alucinação preservada. (−B) Âncora arbitrária + lógica de prompt no TS (dívida a pagar quando o A chegar). (Rejeitado) passar o verbatim de vários trechos pela `question` de uma chamada single-âncora — arrastaria Escritura crua para o input do modelo, exatamente o que a anti-alucinação proíbe.

**Ação requerida (gate).** Sign-off humano escolhendo: **A** (abrir o PR ao `the-light` + re-pin) · **B** (entregar o interino app-side agora, A depois) · **adiar** (manter só o fan-out; validar a Fase 4 no iOS primeiro).

**Atualização (2026-07-07) — ACEITO (Caminho A), entregue SEM tocar o `the-light`.** Sign-off dado ("A — você tem autonomia e tá liberado pra fazer o que precisar no the-light"). Na implementação, a exploração da fronteira revelou um caminho MELHOR que o proposto: `ask_anchored` e cia. vivem no **crate de FRONTEIRA do app** (`core/src/lib.rs`), **não** no `the-light`; e TODOS os helpers puros necessários já são `pub` no core PINADO (`parse_reference`, `numbered_passage`/`numbered_verses`, `ask_context`, `ask`, `build_provider`, `EmbeddedSource::passage`). Logo o prompt CONJUNTO = N blocos `ask_context` **concatenados** (helper `multi_context` — COMPOSIÇÃO de blocos que o core já monta, não prompt novo) + UMA `ai::ask`. Resultado: **o Caminho A saiu INTEIRAMENTE no repo do app, sem PR ao `the-light` e sem re-pin** — novo `ask_multi_anchored` (nativo) + `ai_multi_web_prepare`/`ai_multi_web_finalize` (web, split-transport) + Records `AiAnswerMulti`/`CitedPassage`/`AiPassageInput`/`AiMultiWebRequest`; bindings JSI + wasm regerados; `ScopeStudySheet` troca o fan-out pela síntese conjunta (N trechos citados verbatim + UMA interpretação que os tece). Anti-alucinação **por construção**: o core resolve o verbatim do store, o app só compõe blocos do core; `rewrite_anchors` limpa âncoras espúrias no finalize. Provas: 82 testes Rust (fmt + `clippy -D warnings`), `tsc`, `test:web:ai-multi` (headless: 2 citados verbatim + 1 interpretação + **1 fetch conjunto**, não fan-out), guardas a11y/i18n/mirror-drift/app-const-parity verdes, e verificação no browser (2 trechos disjuntos → 2 blocos citados + 1 síntese). O **Caminho B fica DESCARTADO** — o A foi entregue sem o custo (re-pin/PR ao core) que justificaria o interino. O gate desta ADR está **RESOLVIDO**.

## ADR-0070 — Costura única do SELETOR DE VERSÃO (resolução + carregamento) e da NAVEGAÇÃO de leitura (`version` obrigatório)

- **Data:** 2026-07-10 · **Status:** aceito · **Tarefa:** deepening de arquitetura (`gate: false`) · **Depende:** ADR-0064 (default por idioma), ADR-0060 (molde de hook-costura + redução pura), ADR-0065 (seletor de versão na home). **NÃO** toca o `the-light` nem `core/**` — 100% app-side (TS de UI + puros). Sem mudança de comportamento, sem dependência nova, sem mudança de fronteira. Primeira das ADR-0070…0074 (varredura de deepening do architecture review).

**Contexto.** Três telas repetiam a resolução de versão. A escada `effectiveTranslation` (picked → default-do-idioma → mesmo-idioma → 1ª → default) era **byte-idêntica** na Home (`index.tsx:91-100`) e na Busca (`search/index.tsx:140-149`); o efeito `useState<Translation[]>` + `ensureReadingDb`/`listTranslations` estava **triplicado** (Home, Busca, `useChapterReader`); a derivação de idioma (`searchLang` na busca, `nameLang` no leitor) duplicada; e `HomeVerseOfDay` tinha uma cópia **rogue** de `defaultTranslationFor` que nem importava a compartilhada. Além disso, `version` era montado à mão em **6 sites** de navegação para o leitor — esquecer a linha era exatamente o bug hotfixado hoje (o leitor caía em KJV), e um 7º site (`HomeVerseOfDay`, o toque no versículo-do-dia) ainda descartava a versão silenciosamente.

**Decisão.** Concentrar a superfície atrás de UMA costura, cada tela com sua PRÓPRIA instância (ADR-0064 preservado — a Busca segue o idioma da UI independentemente; um salto para o leitor NÃO muta o seletor da Home; NÃO é um store global):
1. **`app/lib/translationDefault.ts`** ganha as funções PURAS `resolveEffectiveTranslation(picked, translations, locale)` (a escada) e `langForTranslation(id, translations, locale)` (a derivação), com import **só-de-tipo** (`Translation`) → headless-testável (o bundle não boota wasm).
2. **`app/lib/useTranslations.ts`** — carregador único das traduções (o efeito antes triplicado), com `import()` **DINÂMICO** de `db`/`reading` para preservar o code-split da Home (F5.12): importar o hook na Home NÃO puxa a leitura pesada para o entry eager.
3. **`app/lib/useVersionSelection.ts`** — hook-costura que compõe `useTranslations` + `picked` + `effective = resolveEffectiveTranslation(...)`; molde `useChapterReader`. Consumido pela Home e pela Busca (instância local cada).
4. **`app/lib/readingNav.ts`** — `readingChapterHref`/`readingBookHref` puros. `version` é campo **OBRIGATÓRIO** do alvo → o TypeScript IMPEDE abrir o leitor sem carregar a versão (a locality do invariante que era o bug). Os 6 sites (`openHit`/`openRef`/`openBook` na busca, `openXref`/`goToChapter` no leitor, `onSelect` da lista de capítulos) + o `HomeVerseOfDay` (7º, corrigindo o bug latente de versão-descartada no versículo-do-dia) passam pela costura. `useChapterReader` passa a usar `useTranslations` (dedup do 3º efeito).

**Reconciliação com as leis.** *Offline-first/BYOK:* nada muda no transporte — as MESMAS chamadas `listTranslations`/`ensureReadingDb`, `import()` dinâmico preservado; sem rede/conta nova. *Anti-alucinação:* nenhuma lógica de texto tocada — só se resolve QUAL versão exibir; o verbatim segue vindo do store. *Zero drift:* nenhuma lógica de domínio Rust reimplementada — é a MESMA escada/efeito, agora num lugar só. *the-light:* intocado.

**Prova.** `test:web:translation-resolve` (novo, headless node — registrado na lista CI de `run-guards.mjs`) assere a escada de 4 níveis, `langForTranslation` com fallback ao locale, e o invariante "sempre `version`" dos hrefs (verse só quando presente). `tsc` verde; `run-guards all` (48 guardas) verde; smoke em browser `search-version-preserved`/`back-to-home`/`verse-of-day`/`open-chapter`/`chapter-nav`/`search-xref` verdes (comportamento inalterado). **Perf:** re-baseline DELIBERADO — ESTRUTURAL `moduleCount` 911 → 914 (+3 módulos eager EXATOS: `useTranslations`, `useVersionSelection`, `readingNav`; `translationDefault` cresce mas não é módulo novo); BYTES do entry INALTERADOS (3 módulos pequenos, o LOC líquido cai). Espelhado em `scripts/measure-web-bundle.sh` + `loop/perf/web-bundle-budget.json` (`noteADR0070`). (O `perf-budget` roda FORA do CI, local-only; a métrica local acusa drift de ambiente pré-existente — DB de 4 traduções / toolchain local — nas mesmas guardas `reading`/`coverage`; o +3 é o delta estrutural determinístico desta mudança.)

**Consequências.** (+) *Locality:* a escada de resolução, o carregador e o invariante "carregar version" vivem em UM lugar testável; um fix pousa uma vez. (+) *Leverage:* uma interface, 3 telas (hook) + todos os sites de nav (href). (+) A cópia rogue de `HomeVerseOfDay` some e o versículo-do-dia deixa de descartar a versão. (−) Duas indireções a mais (2 hooks + 1 puro) e +3 módulos; mitigado pela testabilidade e pela redução de superfície das telas. `resolveSearchIntent` (a costura de INTENÇÃO de busca — candidato 5 do review) fica para um follow-up no MESMO subsistema (não coube sem inchar o diff). Relacionado: ADR-0059/0060/0061 (mesma varredura de deepening), ADR-0064/0065 (default/seletor), ADR-0047/0050 (política de re-baseline perf).

## ADR-0071 — Costura dos GESTOS de virar-capítulo (teclado/clique-lateral/swipe) + decisões puras em `gestureNav`

- **Data:** 2026-07-10 · **Status:** aceito · **Tarefa:** deepening de arquitetura (`gate: false`) · **Depende:** ADR-0060 (mesma direção de emagrecer a tela do capítulo; molde `hideOnScroll`/`useHideOnScroll`), ADR-0025/0044 (leitura contínua). **NÃO** toca o `the-light` nem `core/**` — 100% app-side. Sem mudança de comportamento, sem dependência nova.

**Contexto.** `app/app/read/[book]/[chapter].tsx` tinha os TRÊS atalhos de virar-capítulo (teclado ←/→, clique-nas-laterais, swipe de toque — todos web-only) como efeitos de `window` INLINE, sem interface. Compartilhavam um guard de seleção de texto TRIPLICADO (`window.getSelection()…length>0`), a checagem de modificador duplicada e a de `reader-body` duplicada; e as DECISÕES puras (cinemática do swipe: distância/duração/dominância-horizontal; e a zona de clique-lateral: margem fora da coluna) só eram alcançáveis simulando eventos de janela — logo, SEM teste (contraste com `chapterNav.ts`/`hideOnScroll.ts`, ambos puros e testados headless).

**Decisão.** Extrair a costura, no molde `hideOnScroll` (puro) + `useHideOnScroll` (hook):
1. **`app/lib/gestureNav.ts`** (puro) — `swipeIntent(dx,dy,dtMs) → 'prev'|'next'|null` (usa `SWIPE_MIN_DISTANCE`/`SWIPE_MAX_DURATION_MS`/`SWIPE_H_DOMINANCE` de `readingLayout`) e `sideNavZone(clientX, viewportWidth, columnMax) → 'prev'|'next'|null` (usa `sideMarginWidth`/`SIDE_NAV_MIN_MARGIN`). As decisões que não tinham teste.
2. **`app/lib/useChapterTurnGestures.ts`** (hook) — possui os 3 listeners de `window` + o ciclo de vida + os guards (navBlocked, campo de texto focado, seleção ativa, alvo interativo, `reader-body`); assinatura `useChapterTurnGestures({ adj, goToChapter, navBlocked, readingColumnMax })`; delega as decisões a `gestureNav`. A tela troca ~92 linhas de 3 efeitos por UMA chamada — "vira capítulo aqui".

**Reconciliação com as leis.** *Offline-first/BYOK:* nenhum I/O — só listeners de `window` (web) e cálculo puro; no nativo é no-op. *Anti-alucinação:* nada de texto tocado — só decide a ROTA do capítulo; o verbatim segue do store. *Zero drift:* nenhuma lógica Rust reimplementada — é o MESMO wiring/decisões, movidos verbatim (a ordem dos guards preservada; mover a checagem de seleção para antes de `swipeIntent` é equivalente — todos são guards sem efeito colateral). *the-light:* intocado.

**Prova.** `test:web:gesture-nav` (novo, headless node — na lista CI) assere `swipeIntent` (rápido+horizontal → prev/next; lento/curto/vertical → null; limiares exatos) e `sideNavZone` (margens → prev/next; meio e tela-estreita → null). `tsc` verde; `run-guards all` (49 guardas) verde; smoke em browser `chapter-nav` (botões + teclado ←/→ + clique-lateral + swipe) + `open-chapter` + `parallel` verdes — os gestos funcionam IDÊNTICOS. **Perf:** re-baseline DELIBERADO — ESTRUTURAL `moduleCount` 914 → 916 (+2 módulos eager: `useChapterTurnGestures` + `gestureNav`); BYTES do entry INALTERADOS. Espelhado em `scripts/measure-web-bundle.sh` + `web-bundle-budget.json` (`noteADR0071`).

**Consequências.** (+) *Locality:* o wiring dos 3 listeners + os guards num lugar; a tela diz só "vira capítulo aqui". (+) *A interface é a superfície de teste:* a cinemática do swipe/zona finalmente tem teste headless (antes zero). (−) +2 módulos e uma indireção; mitigado pela testabilidade. `useImmersiveChrome` (a animação da barra-overlay — candidato "worth exploring") fica DEFERIDO: tem UM só caller (a deletion-test move a complexidade, não concentra de N chamadores) — é coesão/legibilidade, não deepening smeared-across-callers. Relacionado: ADR-0060 (mesma varredura), ADR-0047/0050 (re-baseline perf).

## ADR-0072 — Boot ÚNICO do wa-sqlite (`openSqliteMemVfs`) + correção do VAZAMENTO de handle no `deepStudy`

- **Data:** 2026-07-10 · **Status:** aceito · **Tarefa:** deepening de arquitetura (`gate: false`) · **Depende:** ADR-0011/0012 (store web wa-sqlite/OPFS), ADR-0044 (léxico on-demand), ADR-0057 (leitura = fetch→MemoryVFS). **NÃO** toca o `the-light` nem `core/**` — 100% app-side. Sem mudança de comportamento (exceto a correção do vazamento).

**Contexto.** O boot do wa-sqlite — instanciar o factory SÍNCRONO FTS5 com os bytes do `.wasm` + o workaround DEV `locateFile` do Metro (F5.39: sob o bundler DEV `import.meta.url` é "null" → `new URL(...)` lança), registrar um `MemoryVFS` hidratado com os bytes do DB, e abrir READONLY (com o cast `as unknown as SQLiteVFS`) — era re-tipado **byte-a-byte** nos DOIS openers de produção (`sqlite-reading-opfs.web.ts` e `sqlite-lexicon-opfs.web.ts`), variando só na AQUISIÇÃO de bytes (leitura: `fetch` do asset; léxico: cache OPFS). Além disso, `deepStudy` (`reading.web.ts`) abria DOIS handles FORA do `try` — se `openLexiconDbWeb()` lançasse, o `handle` de leitura já aberto **VAZAVA** (nunca era fechado).

**Decisão.** (1) Extrair **`app/web/sqliteMemVfs.web.ts`** — `openSqliteMemVfs({ wasmBytes, dbBytes, dbName, locateFile? }) → { sqlite3, db, close }`, o boot que vive UMA vez; os 2 openers de produção viram "obter bytes → `openSqliteMemVfs`" (o `locateFile` DEV é opcional). A AQUISIÇÃO de bytes (fetch do asset · cache OPFS · readFile em prova) é o ÚNICO adapter. (2) Corrigir o vazamento do `deepStudy` com brackets **ANINHADOS**: `handle` abre, e o try externo garante seu `close` mesmo se `openLexiconDbWeb()` falhar; o try interno fecha o `lexHandle`.

**Reconciliação com as leis.** *Offline-first/BYOK:* o caminho de I/O é IDÊNTICO (fetch do asset local / cache OPFS; sem rede externa) — só o boot foi movido; a leitura segue sem OPFS (ADR-0057), o léxico segue on-demand (ADR-0044). *Anti-alucinação:* nenhum SQL/texto tocado — só o boot do motor. *Zero drift:* nenhuma lógica de domínio Rust reimplementada; o boot é infra do runtime web. *the-light:* intocado.

**Prova.** `tsc` verde (o cast `SQLiteVFS` — tipo ambiente do wa-sqlite — resolve no módulo novo); `run-guards all` (49 guardas) verde (a camada de query — os módulos `sqlite-*.web` — é intocada). **Smoke em Chrome REAL** (o que de fato exercita os openers de produção via OPFS/wasm — as guardas headless usam openers inline próprios): `open-chapter`/`parallel`/`search-xref` (opener de LEITURA), `study-chat-selector`/`interlinear` (opener de LÉXICO + o caminho `deepStudy` corrigido), `notes-persist` (OPFS) — TODOS verdes, sem `pageerror` proibido. **Perf:** SEM re-baseline — `sqliteMemVfs` é alcançado só por `import()` dinâmico (chunk ASYNC, fora do entry eager), então `moduleCount` (que conta eager) NÃO muda.

**Consequências.** (+) *Locality:* o boot do wa-sqlite e o workaround `locateFile` (o mais frágil do runtime web — quebrou a leitura por 3 ciclos, F5.36/38/39) vivem em UM lugar. (+) A interface (`openSqliteMemVfs`) é bem menor que a implementação (o boot). (+) O vazamento de handle do `deepStudy` é fechado. (−) Uma indireção a mais. **Fora de escopo (deferido):** (a) migrar os ~19 openers HEADLESS inline (`openReadingDbInMemory`/`openDbInMemory`/`openLexiconDbInMemory`) para importar `openSqliteMemVfs` — alto churn de harness, baixo valor incremental (são scaffolding de teste, e a byte-source deles é `readFile`); (b) o bracket `withReadingDb`/`withLexiconDb` genérico nas ~12 fns de `reading.web.ts` — o `try/finally` por-função já é CORRETO (o único bug era o `deepStudy`, corrigido); o bracket seria DRY, não correção — mecânico, follow-up. Relacionado: ADR-0057 (o `-opfs` no nome da leitura já é um misnomer pós-ADR-0057), ADR-0059/0060/0071 (mesma varredura).

## ADR-0073 — `prefs.shared` / `keystore.shared`: módulo profundo + adapters de backend por plataforma

- **Data:** 2026-07-10 · **Status:** aceito · **Tarefa:** deepening de arquitetura (`gate: false`) · **Depende:** ADR-0038 (KV de prefs), ADR-0023/0025 (keystore BYOK, chave web session-only), ADR-0051/0054 (molde `snapshotStore.shared`). **NÃO** toca o `the-light` nem `core/**` — 100% app-side. Sem mudança de comportamento.

**Contexto.** Os pares `prefs.ts`/`prefs.web.ts` e `keystore.ts`/`keystore.web.ts` copiavam o MÓDULO INTEIRO (namespacing + a superfície `create*`; e, no keystore, toda a validação BYOK: `assertProvider`/`trim`/rejeição-de-vazia/`listProviders` só-nomes) em AMBOS os leaves, quando só o BACKEND genuinamente difere (nativo: arquivo JSON / `expo-secure-store`; web: `localStorage` / `Map` de sessão). O corpo de `createKeystore` (o invariante de segurança) era BYTE-IDÊNTICO entre os dois, e a `keystore-headless-entry` importa só a `.ts` nativa — então a cópia WEB da lógica de segurança nunca era testada (uma costura de drift). `prefs` não tinha teste algum.

**Decisão.** Pôr o split no ÚNICO ponto que difere — o backend (molde `snapshotStore.shared`):
1. **`app/lib/keystore.shared.ts`** — o módulo profundo (`SUPPORTED_PROVIDERS`/`Provider`/`keyIdFor`/`isSupportedProvider`/`assertProvider`/`SecureBackend`/`Keystore`/`createKeystore(backend)`). `keystore.ts` → só `defaultSecureBackend` (secure-store) + wrappers; `keystore.web.ts` → só `sessionSecureBackend` (Map) + wrappers. Ambos `export *` do shared + `createKeystore(backend)` ligado ao seu backend.
2. **`app/lib/prefs.shared.ts`** — o módulo profundo (`prefIdFor`/`PrefsBackend`/`Prefs`/`createPrefs(backend)`). `prefs.ts` → backend de arquivo JSON; `prefs.web.ts` → backend `localStorage`. Cada leaf re-exporta a superfície e expõe um `createPrefs(backend = <backend da plataforma>)` (binding de uma linha ao factory de shared — preserva os chamadores que chamam `createPrefs()` sem argumento).

**Reconciliação com as leis.** *Offline-first/BYOK:* o backend por plataforma é IDÊNTICO ao de antes (secure-store no device; sessão in-memory no web, ADR-0025; arquivo/localStorage p/ prefs) — só a LÓGICA foi centralizada; a chave web segue session-only, nunca logada, o import nativo segue lazy. *Anti-alucinação:* não toca texto/IA — é infra de storage. *Zero drift:* nenhuma lógica Rust reimplementada; a lógica BYOK é a MESMA, agora numa fonte só. *the-light:* intocado.

**Prova.** `test:keystore` existente (segue importando a `.ts` nativa, que re-exporta de `.shared`) + **`test:web:prefs` NOVO** (headless — prefs não tinha teste; prova namespacing `tla.pref.<key>` + round-trip sobre backend fake) — ambos na lista CI. `tsc` verde; `run-guards all` (50 guardas) verde; smoke em browser `settings` (setKey/deleteKey/listProviders no cofre de sessão web), `study-chat-selector` + `ai-reachability` (getKey) verdes. **Perf:** re-baseline DELIBERADO — ESTRUTURAL `moduleCount` 916 → 918 (+2 módulos eager: `keystore.shared` + `prefs.shared`); BYTES do entry INALTERADOS. Espelhado em `scripts/measure-web-bundle.sh` + `web-bundle-budget.json` (`noteADR0073`).

**Consequências.** (+) *Locality:* um fix de validação BYOK / namespacing pousa UMA vez, não duas em paridade-por-mão; a lógica de segurança da chave web deixa de ser confiada-sem-teste. (+) *Ports & adapters real:* um módulo profundo, dois adapters (o backend é o que varia). (−) +2 módulos e um nível de indireção. **Fora de escopo (deferido):** o collapse de `snapshotStore.ts`/`.web.ts` em `.shared` (os 2 leaves diferem em UMA linha — de onde vem `parseReferenceSync`) exigiria expor `parseReferenceSync` pela costura `reading` (tocar o seam de leitura, mais frágil/parity-locked) — follow-up, para não misturar com a extração de storage. Relacionado: ADR-0051/0054 (`snapshotStore.shared` como molde), ADR-0023/0025 (BYOK), ADR-0059/0060/0071/0072 (mesma varredura).

## ADR-0074 — `AttributionBlock`: atribuição STEP CC-BY numa fonte + render únicos (fim do import painel→painel)

- **Data:** 2026-07-10 · **Status:** aceito · **Tarefa:** deepening de arquitetura (`gate: false`) · **Depende:** ADR-0026 (atribuição STEP CC-BY obrigatória), ADR-0059 (seam de provedor/BYOK já extraído), ADR-0066/0068 (kit Vigil). **NÃO** toca o `the-light` nem `core/**` — 100% app-side. Sem mudança de comportamento. Última das ADR-0070…0074 (varredura de deepening do architecture review).

**Contexto.** A atribuição STEP/Tyndale CC BY 4.0 (ADR-0026, OBRIGATÓRIA onde léxico/interlinear aparece) era duplicada em `ReaderStudyPanel` e `ReaderInterlinearPanel`: a constante canônica `STEP_ATTRIBUTION`, o fallback `attributionLines = sources.length>0 ? sources : [STEP_ATTRIBUTION]`, o render `<View>…map(<Text>)</View>`, e os estilos `attributionBlock`/`attribution` — tudo byte-idêntico. Pior: a constante era **exportada de um painel e IMPORTADA pelo painel irmão** (`ReaderInterlinearPanel` fazia `import { STEP_ATTRIBUTION } from './ReaderStudyPanel'`) — acoplamento componente→componente.

**Decisão.** (1) **`app/lib/attribution.ts`** (puro) — `STEP_ATTRIBUTION` + `attributionLinesFrom(sources)` (o fallback). (2) **`app/components/ui/AttributionBlock.tsx`** — `<AttributionBlock sources testID />`, o render tokenizado. Os 2 painéis passam a satisfazer UMA interface; o import painel→painel some. A verificação de licença (`about-attributions` guarda) passa a ler `lib/attribution.ts` (a constante mudou de arquivo, não de valor).

**Reconciliação com as leis.** *Anti-alucinação/licença:* o requisito de atribuição NUNCA cai — `attributionLinesFrom` exibe as `sources` REAIS do retorno (verbatim do banco) e só cai na canônica se vierem vazias; a string canônica é byte-idêntica à anterior (e ao catálogo i18n `about.stepAttribution`, provado pela guarda). *Offline-first/BYOK:* não toca chave/rede. *Zero drift:* nenhuma lógica Rust; a atribuição vem do store. *the-light:* intocado.

**Prova.** `test:about-attr` (re-apontado p/ `lib/attribution.ts`) prova `STEP_ATTRIBUTION` == `about.stepAttribution` (pt/en idênticos, STEP CC BY 4.0). `tsc` verde; `run-guards all` (50 guardas) verde; smoke em browser `study-chat-selector` + `interlinear` (o `<AttributionBlock>` renderiza nos dois) verdes. **Perf:** re-baseline DELIBERADO — ESTRUTURAL `moduleCount` 918 → 920 (+2 módulos eager: `lib/attribution` + `ui/AttributionBlock`); BYTES do entry INALTERADOS. Espelhado em `scripts/measure-web-bundle.sh` + `web-bundle-budget.json` (`noteADR0074`).

**Consequências.** (+) *Locality:* o invariante de licença (ADR-0026) vive num só lugar; o import painel→painel some. (+) *Leverage:* uma interface, 2 painéis (e disponível p/ futuros). (−) +2 módulos. **Fora de escopo (deferido):** o `<ProviderGate>` (candidato "worth exploring") — concentrar o gate de `<AiProviderNotice>` + a CTA `needsKey` + o handler `onConfigureProvider` (duplicados nos painéis de IA) — fica adiado: os blocos são renderizados em POSIÇÕES diferentes por painel (notice perto dos chips; CTA na área de ação) com nomes de estilo próprios (`needKeyBlock`/`needKey`), e Ask/Compare são exceções documentadas (Ask sem CTA; Compare multi-select com chrome próprio) — a unificação renderia menos que o `AttributionBlock` e com mais risco de regressão de a11y/layout; melhor como follow-up dedicado. Relacionado: ADR-0026 (licença), ADR-0059 (seam de provedor já extraído; o render é o que resta), ADR-0070…0073 (mesma varredura).

## ADR-0075 — `resolveDidYouMean`: orquestração PURA do "você quis dizer?" (zero-path da busca), preservando o timing

- **Data:** 2026-07-10 · **Status:** aceito · **Tarefa:** deepening de arquitetura (`gate: false`) · **Depende:** ADR-0064 (busca inteligente app-side; `buildDidYouMean` com ports injetados). **NÃO** toca o `the-light` nem `core/**` — 100% app-side. Sem mudança de comportamento (timing INALTERADO).

**Contexto.** O architecture review apontou a tela de busca (`search/index.tsx`) como um candidato Strong: a ORQUESTRAÇÃO (debounce, race-guard `seqRef`, zero-path) vive em efeitos e NÃO tem superfície de teste — enquanto os helpers puros (`fold`, `buildDidYouMean`, `suggestFuzzy`…) já são fundos e testados. A parte NÃO-trivial e sem teste era a montagem do caminho ZERO-resultado, INLINE no efeito principal: a SONDA de existência (`search(cand, translation, limit=1) → .length`, `catch→0`), os candidatos fuzzy do corpus (`suggestFuzzy(...).catch→[]`) e a composição em `buildDidYouMean` — o fio das portas e os `catch` de resiliência, exatamente onde bugs de orquestração se escondem.

**Decisão (escopo FOCADO, timing-preservador).** Extrair só a montagem do zero-path para uma função PURA — **`app/lib/searchIntent.ts` → `resolveDidYouMean(term, translation, locale, lang, ports)`**, com `ports = { search, suggestFuzzy }` injetados. A tela injeta as portas (`search` fechando sobre o `dbPath`; `suggestFuzzy`) e aplica o resultado sob o race-guard. **Deliberadamente NÃO se mesclam os 4 efeitos** (autocomplete de palavra 120ms, sugestão de livro síncrona, detecção de referência 300ms, busca principal 300ms) nem se altera a estrutura de DUAS FASES (`setResults` imediato após a busca; `setSuggestions` depois, no zero-path) — mesclar mudaria o timing do "sem resultados" (apareceria após a sondagem). O debounce, o `seqRef` e as duas fases seguem no efeito → **timing byte-a-byte inalterado**; só a orquestração pura saiu.

**Reconciliação com as leis.** *Offline-first/BYOK:* nenhuma rede nova — a sonda é a MESMA busca local (limit=1). *Anti-alucinação:* a busca EXATA é intocada; só sugere termos que RETORNAM resultados do store (nunca inventa). *Zero drift:* nenhuma lógica Rust reimplementada — é a MESMA montagem, agora pura/testável; as portas mapeiam verbatim para as chamadas anteriores (`search(dbPath,…,undefined,limit)`, `suggestFuzzy(…,6)`). *the-light:* intocado.

**Prova.** `test:web:search-intent` (novo, headless — na lista CI) injeta portas fake e prova: só candidatos que retornam resultados sobrevivem (o typo é filtrado; a fuzzy que existe é sugerida); e a resiliência (sonda que lança → 0; `suggestFuzzy` que lança → []) não quebra. `tsc` verde; `run-guards all` (51 guardas — inclui `search-smart`, que prova o did-you-mean end-to-end sobre o store real) verde; smoke em browser `search-version-preserved` + `search-xref` verdes (a tela busca/renderiza igual). **Perf:** re-baseline DELIBERADO — ESTRUTURAL `moduleCount` 920 → 921 (+1 módulo eager: `searchIntent`); BYTES do entry INALTERADOS. Espelhado em `scripts/measure-web-bundle.sh` + `web-bundle-budget.json` (`noteADR0075`).

**Consequências.** (+) *Locality + a interface é a superfície de teste:* o fio das portas e os `catch` do zero-path ganham teste headless (antes zero). (+) Baixo risco: o timing/duas-fases é preservado exatamente. (−) +1 módulo. **Fora de escopo (deferido, com razão):** o `useSearchIntent` COMPLETO (um hook possuindo os 4 efeitos + o `seqRef` num só lugar) — mesclar os 4 efeitos com debounces DIFERENTES (120ms vs 300ms) arriscaria mudar timing/UX de forma sutil; a costura pura aqui já dá teste à parte que importava (a orquestração do zero-path) sem esse risco. Fecha a varredura de deepening ADR-0070…0075 do architecture review. Relacionado: ADR-0064 (busca inteligente), ADR-0070 (a costura de versão desta mesma tela), ADR-0060 (molde de hook + seam pura).

## ADR-0076 — `useImmersiveChrome`: o ciclo da barra-overlay imersiva numa costura (coesão)

- **Data:** 2026-07-10 · **Status:** aceito · **Tarefa:** deepening de arquitetura (`gate: false`) · **Depende:** ADR-0069-feel (leitura imersiva), ADR-0071 (mesma tela; `useHideOnScroll` como base). **NÃO** toca o `the-light` nem `core/**` — 100% app-side. Sem mudança de comportamento. Follow-up autorizado (o architecture review o marcou "worth exploring / deferido").

**Contexto.** A leitura imersiva tinha, soltos na tela do capítulo, QUATRO pedaços de estado co-variantes (`barHeight`, `chromeAnim`, `chromeGone`, `chromeHidden`) + DOIS efeitos interligados (o slide `translateY`+`opacity` com curvas assimétricas + reduce-motion; e o reset da barra quando uma âncora de busca/xref chega) + ~20 linhas de interpolação inline no render. A deletion-test é honesta: há UM só caller (o leitor imersivo), então extrair MOVE a complexidade, não a concentra de N chamadores — é COESÃO/legibilidade, não deepening smeared-across-callers (por isso o review o pôs em "worth exploring", não Strong).

**Decisão.** `app/lib/useImmersiveChrome.ts` — hook que possui o ciclo `hidden→anim→gone` (sobre o `useHideOnScroll` já profundo), a medição (`onLayout`/`barHeight`), o reduce-motion e o reset-por-âncora; devolve `{ barHeight, onReaderScroll, resetChrome, overlayProps, gone, animatedStyle }`. A tela renderiza `<Animated.View {...overlayProps} style={[styles.chrome, {paddingTop: insets.top}, gone ? styles.chromeGone : null, animatedStyle]}>` em vez de open-codar a interpolação; `barHeight`/`onReaderScroll`/`resetChrome` seguem com os mesmos nomes → `goToChapter`, os corpos de leitura e o `topInset` ficam INALTERADOS.

**Reconciliação com as leis.** Nenhum I/O/rede/texto — só estado de animação e medição de UI. *Anti-alucinação/offline/BYOK/the-light:* intocados.

**Prova.** `tsc` verde; `run-guards all` (51 guardas — inclui `a11y-modals`/`hide-on-scroll`) verde; smoke em browser `open-chapter` + `chapter-nav` (o leitor renderiza; virar capítulo usa `resetChrome`) verdes. **Perf:** re-baseline DELIBERADO — ESTRUTURAL `moduleCount` 921 → 922 (+1 módulo eager); BYTES do entry INALTERADOS. Espelhado em `scripts/measure-web-bundle.sh` + `web-bundle-budget.json` (`noteADR0076`).

**Consequências.** (+) A tela do capítulo perde ~40 linhas de estado/efeito/interpolação de chrome; o ciclo co-variante vive num lugar coeso. (−) +1 módulo e uma indireção; sem headless-test (o ciclo é estado/animação de React, não puro — a base `hideOnScroll` já é testada). Relacionado: ADR-0071 (gestos da mesma tela), ADR-0060 (molde de hook).

## ADR-0077 — Brackets `withReadingDb`/`withLexiconDb`: política de fechamento de handle numa costura

- **Data:** 2026-07-10 · **Status:** aceito · **Tarefa:** deepening de arquitetura (`gate: false`) · **Depende:** ADR-0040 (code-split F5.9), ADR-0072 (opener único + o fix do vazamento do `deepStudy`). **NÃO** toca o `the-light` nem `core/**` — 100% app-side. Sem mudança de comportamento.

**Contexto.** Em `app/web/reading.web.ts`, ~12 funções repetiam o ciclo `open handle → try → delegar a *OnHandle → finally close`. Um `finally` esquecido VAZA o handle wa-sqlite — e não é hipotético: o `deepStudy` teve exatamente esse bug (abria dois handles fora do try; a ADR-0072 o corrigiu à mão com brackets aninhados). A política de fechamento estava espalhada por 12 chamadores, não concentrada.

**Decisão.** Dois brackets privados — `withReadingDb(fn)` / `withLexiconDb(fn)` — que abrem o store (via `import()` dinâmico, preservando o code-split F5.9), rodam `fn(handle)` e SEMPRE fecham no `finally` (leak-safe POR CONSTRUÇÃO). Cada função vira "importa seu `*OnHandle` → `return withReadingDb(handle => xOnHandle(handle, …))`". O `deepStudy` COMPÕE os dois — `withReadingDb(handle => withLexiconDb(lex => deepStudyOnHandle(handle, lex, …)))` — o que expressa estruturalmente o que a ADR-0072 corrigiu à mão: o léxico fecha antes da leitura, e se o léxico falhar ao ABRIR, o bracket externo fecha a leitura (sem vazar).

**Reconciliação com as leis.** *Offline-first:* o I/O é idêntico (mesmos openers, `import()` dinâmico). *Anti-alucinação:* nenhum SQL/texto tocado — só o ciclo de vida do handle. *Zero drift:* nenhuma lógica de domínio Rust reimplementada; os `*OnHandle` (o espelho parity-locked) ficam INTOCADOS. *the-light:* intocado. (Nota: os 2 `import()` — opener no bracket, `*OnHandle` na função — deixam de ser `Promise.all` paralelo; ambos os módulos são cacheados após a 1ª chamada, então o custo é ~1 microtask só no cold-start da 1ª chamada de cada função — negligível.)

**Prova.** `tsc` verde; `run-guards all` (51 guardas) verde (a camada `*OnHandle` — o que as guardas headless testam — é intocada). **Smoke em Chrome REAL** (o que exercita os wrappers de `reading.web.ts` de fato): `open-chapter`/`parallel` (`getChapter`), `search-xref` (`search`/`crossRefs`), `study-chat-selector` (`deepStudy` composto), `interlinear` (`interlinearVerse`), `notes-persist`, `plans-persist`, `ai-reachability` (`askAnchored`) — verdes, sem `pageerror` proibido. **Perf:** SEM re-baseline — os brackets são funções DENTRO de `reading.web.ts` (nenhum módulo eager novo); `moduleCount` não muda.

**Consequências.** (+) *Locality:* a política de fechamento vive em UM lugar; um chamador NÃO pode esquecer o `finally` (leak-safe por construção). (+) A correção manual do `deepStudy` (ADR-0072) vira grátis pela composição. (+) Cada função encolhe para a sua linha distintiva. (−) Uma indireção de closure a mais; a micro-perda de paralelismo dos 2 `import()` no cold-start (negligível, módulos cacheados). Relacionado: ADR-0072 (opener + o vazamento que isto torna estrutural), ADR-0040 (code-split preservado).

## ADR-0078 — Collapse do `snapshotStore` (2 leaves → 1) expondo `parseReferenceSync` pela costura de leitura

- **Data:** 2026-07-10 · **Status:** aceito · **Tarefa:** deepening de arquitetura (`gate: false`) · **Depende:** ADR-0054 (`snapshotStore.shared` como adaptador puro), ADR-0011 (costura de leitura web/nativo). **NÃO** toca o `the-light` nem `core/**` — 100% app-side. Sem mudança de comportamento.

**Contexto.** `snapshotStore.ts` (nativo) e `snapshotStore.web.ts` (web) duplicavam TODO o `createRealSnapshotStore` (a memoização de `bookNameEn` + o wiring completo das deps de `createSnapshotStore`), diferindo em UMA linha: de onde vem o `parseReferenceSync` (nativo: `../web/native-generated`; web: `../web/generated/the_light_app_core`). Todas as OUTRAS deps (`listBooks`/`listNotes`/… de `../web/reading`) já eram o MESMO import nos dois — porque `web/reading` é a costura de plataforma. Só a validação de referência (o `parse_reference` síncrono) não estava exposta pela seam.

**Decisão.** Expor `parseReferenceSync` (o `parse_reference` SÍNCRONO do core) pela costura de leitura: `reading.ts` re-exporta o binding JSI, `reading.web.ts` re-exporta o do wasm. Então `snapshotStore.ts` passa a importar `parseReferenceSync` + as fns de leitura TODAS de `../web/reading` (a seam) e vira PLATFORM-AGNOSTIC; `snapshotStore.web.ts` é DELETADO (o Metro resolve `../lib/snapshotStore` para o único arquivo restante, que já resolve a plataforma pela seam).

**Reconciliação com as leis.** *Anti-alucinação:* a validação REAL de referência (`parse_reference` do core, que lança em referência irreal antes de escrever) é preservada — agora vinda pela seam, o MESMO binding síncrono. *Offline-first/BYOK:* nada muda no I/O; carregado on-demand (fora do 1º paint). *Zero drift:* nenhuma lógica reimplementada — é o MESMO wiring, num só arquivo. *the-light:* intocado.

**Prova.** `tsc` verde; `run-guards all` (51 guardas — inclui `snapshot`/`syncui`/`drivesync`; o motor PURO `snapshotStore.shared`/`userdataSnapshot` que eles testam é INTOCADO) verde; smoke em browser `export-import` + `import-fresh` (round-trip de snapshot via `createRealSnapshotStore` colapsado) + `notes-persist` verdes. **Perf:** SEM re-baseline — `snapshotStore` é `import()` on-demand (chunk async, não eager); deletar um leaf não muda o `moduleCount` do entry.

**Consequências.** (+) *Locality:* o wiring do store vive num só arquivo; uma mudança de método pousa uma vez, não duas em paridade-por-mão. (+) `parseReferenceSync` na seam de leitura fica disponível a outros consumidores app-side. (−) A costura de leitura ganha +1 símbolo `pub` (o `parseReferenceSync`) — superfície mínima. Relacionado: ADR-0073 (mesma disciplina `.shared`/seam), ADR-0054 (o adaptador puro que os leaves ligavam).

## ADR-0079 — Chrome de provedor/BYOK dos painéis de IA: `ProviderNeedKeyCta` + `goToProviderSettings`

- **Data:** 2026-07-10 · **Status:** aceito · **Tarefa:** deepening de arquitetura (`gate: false`) · **Depende:** ADR-0059 (o SEAM de provedor/BYOK já extraiu o ESTADO; isto é o RENDER). **NÃO** toca o `the-light` nem `core/**` — 100% app-side. Sem mudança de comportamento (uma normalização de a11y deliberada). Realiza o `<ProviderGate>` que a ADR-0074 deixou deferido.

**Contexto.** A ADR-0059 concentrou o ESTADO de provedor/BYOK (`useProviderSelection`), mas o CHROME derivado dos seus booleanos seguia hand-renderizado nos painéis: (1) o handler `onConfigureProvider` — `onClose(); router.push('/settings')` — era copiado BYTE-A-BYTE nos 5 painéis (Ask/Chat/Study/Compare/Scope); (2) o bloco CTA "provedor real sem chave" (View + erro `t('ask.needKeyError')` + Button p/ Ajustes) era duplicado em Study/Chat/Scope, variando só no `testID`, na margem do botão e — no Scope — num `accessibilityLabel` FALTANDO no botão.

**Decisão.** (1) **`app/lib/aiConfigure.ts` → `goToProviderSettings(onClose)`** — o handler, num lugar; os 5 painéis passam a `const onConfigureProvider = () => goToProviderSettings(onClose)`. (2) **`app/components/ProviderNeedKeyCta.tsx`** — o bloco CTA (View+erro+Button), consumido por Study/Chat/Scope; o painel decide QUANDO (`{needsKey ? <ProviderNeedKeyCta/> : null}`). A margem do botão (Study `md` vs Chat/Scope `sm`) vem por `buttonStyle` → paridade EXATA; o Scope ganha o `accessibilityLabel` que faltava (normalização de a11y, alinhado a Study/Chat). **Ask** (sem CTA — usa save-de-chave inline) e **Compare** (multi-select, chrome próprio) permanecem exceções documentadas (ADR-0059); o gate do `<AiProviderNotice>` (já um one-liner sobre o componente compartilhado) fica como está.

**Reconciliação com as leis.** *BYOK/anti-alucinação:* o chrome não toca chave/`AiAnswer` — só o botão de navegar p/ Ajustes; a chave segue lida sob demanda, nunca logada (ADR-0025/0059). *Offline-first:* só navegação. *Zero drift:* nenhuma lógica de domínio; é o MESMO render/handler, num lugar só. *the-light:* intocado.

**Prova.** `tsc` verde; `run-guards all` (51 guardas — `a11y-scan`/`a11y-modals`/`i18n`/`contrast` provam que mover o chrome + o novo label do Scope não regridem a11y/rótulos) verde; smoke em browser `study-chat-selector` + `ai-reachability` verdes. **Perf:** re-baseline DELIBERADO — ESTRUTURAL `moduleCount` 922 → 924 (+2 módulos eager: `ProviderNeedKeyCta` + `aiConfigure`); BYTES do entry INALTERADOS. Espelhado em `scripts/measure-web-bundle.sh` + `web-bundle-budget.json` (`noteADR0079`).

**Consequências.** (+) *Locality:* o CTA de "sem chave" e o handler de "configurar" vivem num lugar; o Scope deixa de estar sem `accessibilityLabel`. (+) Deepena o RENDER do seam ADR-0059 (um adapter alimentado pelo hook — NÃO o mega-hook que a ADR-0059 rejeitou). (−) +2 módulos; a margem do botão fica como prop (a única diferença real entre os painéis). Relacionado: ADR-0059 (o seam de estado), ADR-0074 (que deferiu este render), ADR-0026/0068 (kit/anti-alucinação dos painéis).

## ADR-0080 — `useSearchIntent`: os 4 produtores da busca + o estado numa costura (preservando os debounces distintos)

- **Data:** 2026-07-10 · **Status:** aceito · **Tarefa:** deepening de arquitetura (`gate: false`) · **Depende:** ADR-0064 (busca inteligente app-side), ADR-0075 (`resolveDidYouMean`, a costura pura que este hook reusa), ADR-0060 (molde de hook-costura). **NÃO** toca o `the-light` nem `core/**` — 100% app-side. Sem mudança de comportamento. Realiza o `useSearchIntent` COMPLETO que a ADR-0075 deferiu (por risco de timing).

**Contexto.** A tela de busca orquestrava QUATRO produtores assíncronos com debounces e deps DIFERENTES — autocomplete de termo (120ms), sugestão de livro (memo síncrono), detecção de referência (300ms), busca principal + "você quis dizer?" (300ms + race-guard `seqRef`) — mais 6 pedaços de estado de busca, todos soltos na tela, sem interface. A ADR-0075 já extraiu a orquestração do zero-path (`resolveDidYouMean`) mas deixou os 4 efeitos na tela, deferindo o hook completo por medo de mudar o timing ao mesclar os debounces distintos.

**Decisão.** `app/lib/useSearchIntent.ts` — hook que POSSUI os 4 produtores (cada um com o SEU debounce/deps — NÃO se mesclam) + o estado (`results/loading/error/didYouMean/wordSuggestions/parsedRef/bookSuggestions`), reusando a costura pura `resolveDidYouMean`. Assinatura `useSearchIntent(term, translation, lang, locale, books)`. A tela injeta esses 5 e RENDERIZA a partir do retorno — some ~90 linhas de efeitos/estado. Única normalização: a busca principal passa a keyar por `term` (trimado) como os outros 3 produtores, em vez de `query` cru — MESMOS resultados, só evita uma re-busca redundante em edições só-de-espaço.

**Reconciliação com as leis.** *Anti-alucinação:* a busca EXATA (FTS5) é intocada; o "você quis dizer?" só sugere termos que RETORNAM resultados do store. *Offline-first/BYOK:* nenhuma rede nova. *Zero drift:* nenhuma lógica Rust reimplementada — é o MESMO wiring, num lugar só; os debounces/o race-guard preservados. *the-light:* intocado.

**Prova.** `tsc` verde; `run-guards all` (51 guardas — `search-smart` prova o did-you-mean end-to-end no store real; `search-intent` da ADR-0075 prova a orquestração pura que o hook reusa) verde; smoke em browser `search-version-preserved` + `search-xref` + `back-to-home` verdes (a tela busca/renderiza/clica igual). **Perf:** re-baseline DELIBERADO — ESTRUTURAL `moduleCount` 924 → 925 (+1 módulo eager); BYTES do entry INALTERADOS. Espelhado em `scripts/measure-web-bundle.sh` + `web-bundle-budget.json` (`noteADR0080`).

**Consequências.** (+) *Locality:* os 4 produtores + o estado de busca vivem num lugar; a tela vira quase só render. (+) O race-guard e os debounces distintos ficam num só arquivo, não espalhados. (−) +1 módulo e uma indireção; o hook NÃO é headless-testado diretamente (é estado/efeito de React — mas a parte não-trivial, `resolveDidYouMean`, já é testada). Fecha a família de deepenings da busca (ADR-0070/0075/0080). Relacionado: ADR-0075 (a costura pura reusada), ADR-0064 (busca inteligente), ADR-0060 (molde).
