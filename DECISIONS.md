...
- **Fatias RESTANTES — cada uma carrega UMA DECISÃO (não é refactor byte-a-byte trivial; por isso a fatia 1 parou na parte mecânica limpa).** Achado ao mapear:
  ...
  - **`extractors` (não-stream)** (F-b) — sucesso idêntico, MAS as mensagens de ERRO divergem: o core embrulha em `AiError::BadResponse` → `#[error("resposta inesperada do provedor: {0}")]`, enquanto o TS lança a string CRUA (`"o modelo recusou a solicitação (refusal)"`, `"resposta de texto vazia"`…). O usuário ACEITOU a mudança (mensagem mais clara). Próxima fatia (com os deltas).
