...
- **Conclusão (2026-07-10, após 9 PRs — 3 ao the-light, 6 ao app, todos verdes):** os TRÊS seams da ADR foram colapsados ao endpoint DATA-ONLY:
  ...
  - **SQL — leitura/busca/xref/passage 100%** (via o módulo `query` + `SqlParam` no core; o web executa `{sql, params}`). **Pendente:** só a fatia LEXICON (`ai/lexicon.rs` collect/interlinear/attributions) — mesma mecânica já provada (planner de SQL + shaper/agregação mantido no TS), num PR análogo ao #8/#47; é o único espelho SQL restante (guardado por `mirror-drift`).
  - **Portão wasm-safe** (`wasm-pure`) adicionado ao CI do the-light; anti-alucinação/BYOK preservados em todas as fatias. Status: **EM IMPLEMENTAÇÃO (transporte+formatos+SQL-leitura entregues; lexicon a completar).**
