...
- **Conclusão:** a fatia mecânica limpa (URLs + `max_tokens`) está entregue e provada ponta-a-ponta (the-light PR + re-pin + frontier + regen + migração + CI verde nos 2 repos). As demais fatias são bem-escopadas mas cada uma pede uma decisão de produto (aceitar texto de erro/streaming estrito) ou um novo PR de core (SqlParam/slug) — trabalho focado, não continuação apressada. Status: **PROPOSTO → EM IMPLEMENTAÇÃO (fatia 1 de N entregue).**
